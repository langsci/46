...
Es gibt foklgerichtig zwei Varianten für die Verschriftung von /\textipa{\u{E}}/, nämlich \textit{e} (\textit{Bett}) und \textit{ä} (\textit{Säcke}).
Zusätzlich \textit{e} für /\textipa{@}/ verwendet.
Im Fall der gespannten /\textipa{e E}/ und der zwei ungespannten /\textipa{\u{E}}/ ist die Buchstabenschreibung also hochgradig konsequent, und eventuelle Verwirrung kommt nur daher, dass im phonologische System zwei gespannte Vokale mit demselben ungespannten Vokal korrespondieren.
...
\textit{quasseln} sollte angesichts der Schreibung und der phonotaktischen Regularitäten analog zu \textit{prasseln} \textipa{[pKa\Sgel{s}@ln]} als \textipa{[kva\Sgel{s}@ln]} realisiert werden, bei vielen Sprechern wird es aber als \textipa{[kva\Sgel{z}@ln]} (mit einem eigentlich nicht möglichen stimmhaften Silbengelenk) realisiert.
...
Die Nicht-Verfügbarkeit von zwei Buchstaben für /s/ und /s/ suggeriert, dass wir zumindest für den Kern des phonologischen Systems ohne eine Opposition von /\textipa{s}/ und /\textipa{z}/ auskommen sollten, zumal die Segmente eigentlich nahezu komplementär verteilt sind und damit eventuell auf ein zugrundeliegendes Segment reduziert werden können.
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex[*]{\label{ex:eszett003b} \textipa{[bU\Sgel{z}@n]}}
  \end{xlist}
  }
\end{exe}

Weiterhin nehmen wir für die problematischen Wörter wie \textit{Bußen} wir eine zugrundeliegende Form mit zwei /z/ an, also /\textipa{buzz@n}/.
