...
Allerdings gibt es Positionen im Wort, in denen sie distinktiv sind, und in denen das \textit{ß} bei der Unterscheidung zwischen /\textipa{s}/ und /\textipa{z}/ hilft, \zB \textit{Muße} /\textipa{mu:s@}/ und \textit{Muse} /\textipa{mu:z@}/, was in Abschnitt~\ref{sec:laengeschreib} genauer erklärt wird.
...
Sie zeigen die Kürze des Vokals an.
\index{Schärfungsschreibung}
}
...
Aus den Regularitäten des Silbengewichts kann man ebenfalls ableiten, warum in Wörtern vom Typ \textit{Lippe} die Schärfungsschreibung steht, in denen vom Typ \textit{Rinde} allerdings nicht.
...
Die Schärfungsschreibung ist also eine Silbengelenkschreibung, und in den Beispielen in Tabelle~\ref{tab:dehnkuerzschreib} kodiert \textit{Lippe} \textipa{[lI\Sub{p}.p@]}, \textit{Futter} \textipa{[fU\Sub{t}.t5]}, \textit{Wecke} \textipa{[vE\Sub{k}.k@]}, \textit{offen} \textipa{[O\Sub{f}.f@n]} und \textit{wacker} \textipa{[va\Sub{k}.k5]}.%
...
Wollte man hier mit Silbengelenk silbifizieren, käme nur *\textipa{[KI\Sub{n}.nd@]} in Frage, was als *\textit{Rinnde} zu verschriften wäre.
...
\footnote{Letzteres könnte mit Bezug auf Gelenk- und Konstantschreibung erklärt werden, wie in Abschnitt~\ref{sec:laengeschreib} angedeutet.
...
\begin{figure}
  ...
  \Tree{
    ...
    \K{\textipa{b}} & \K{\textipa{u:}} & \K{\textipa{s}} & \K{\textipa{z}} & \K{\textipa{@}} & \K{\textipa{n}} \\
  }
...
Im Wort \textit{Bussen} \textipa{[bU\Sub{s}.s@n]} ist der Vokal der Erstsilbe kurz, die Silbe aber dank Silbengelenk geschlossen und damit zweimorig.
Die Silbengelenkschreibung \textit{ss} zeigt genau dies an.
In solchen Wörtern sollte ein /\textipa{z}/ nicht möglich sein, denn durch die Silbengelenkposition steht das Segment ja stets in einem Endrand, in dem die Auslautverhärtung wirkt und jedes /\textipa{z}/ zu \textipa{[s]} macht (s.\ auch Abschnitt~\ref{sec:einsilblerzweisilbler}, besonders S.~\pageref{abs:silbengelenkstimmlos}).
...
\textit{quasseln} sollte angesichts der Schreibung und den phonotaktischen Regularitäten analog zu \textit{prasseln} \textipa{[pKas@ln]} als \textipa{[kvas@ln]} realisiert werden, bei vielen Sprechern wird es aber als \textipa{[kvaz@ln]} realisiert.
Im Unterschied zu den in Abschnitt~\ref{sec:einsilblerzweisilbler} erwähnten Wörtern wie \textit{Bagger} und \textit{Robbe} hat man bei \textit{quasseln} den erheblichen Nachteil, dass für /\textipa{s}/ und /\textipa{z}/ nur ein Buchstabe zur Verfügung steht, für /\textipa{k}/ und /\textipa{g}/ usw.\ aber zwei.
Die Verfügbarkeit der Buchstaben suggeriert, dass wir auch für den Kern des phonologischen Systems ohne eine Opposition von /\textipa{s}/ und /\textipa{z}/ auskommen sollten, weil sie eigentlich komplementär verteilt sind und damit ein zugrundeliegendes Segment reduziert werden können.
Hier wird argumentiert, dass sie das auch sind.

Im Fall von \textit{Bußen} ist der Vokal lang, und in der Silbifizierung \textipa{[bu:.s@n]} erwarten wir daher prinzipiell kein Silbengelenk.%
...
Wie oben erwähnt stört die Silbe \textipa{[s@n]} aber im System, weil stimmloses \textipa{[s]} hier im Anfangsrand steht.
Wir setzen die zugrundeliegende Form als /\textipa{buss@n}/ an und nehmen außerdem eine Strukturbedingung an, die verlangt, dass /s/ im Anfangsrand als \textipa{[z]} realisiert wird.
Zugrundeligend für \textit{Saal} ist damit die Form /\textipa{sal}/, die phonetisch als \textipa{[za:l]} realisiert wird, parallel für \textit{Senf} /\textipa{s\u{E}nf}/ und \textipa{[zEnf]} usw. 
Damit ergäbe sich für \textit{Bußen} eigentlich *\textipa{[bu:s.z@n]}.
In dieser Konstellation müsste das \textipa{[z]} in seinem Stimmton an das \textipa{[s]} angepasst (\textit{assimiliert}) und stimmlos werden.
Das phonetische Ergebnis ist \textipa{[bu:s.s@n]}, und die Opposition zwischen /\textipa{s}/ und /\textipa{z}/ ist abgeschafft.
Das Eszett kodiert in dieser Analyse einerseits die zwei zugrundeliegenden /s/ und stellt gleichzeitig eine Dehnungsschreibung dar, wie die unterste Struktur in Abbildung~\ref{fig:busen} zeigt.
