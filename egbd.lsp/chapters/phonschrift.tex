...
\footnote{Das gleiche gilt für die im Standard übliche Realisierung /\textipa{Ig}/ als \textipa{[I\c{c}]} im Wortauslaut.
Vergleiche die Formen \textit{wenig} /\textipa{venIg}/ \textipa{[ve:nI\c{c}]} und \textit{weniger} /\textipa{venIg@K}/ \textipa{[ve:nIg5]}.}
...
Das \textit{e} verschriftet das gespannte /\textipa{e}/ (\textit{wenig}), das gespannte /\textipa{E}/ (\textit{spät}), das ungespannte /\textipa{\u{E}}/ (\textit{Bett}) sowie Schwa.
Der Buchstabe \textit{ä} korrespondiert ebenfalls mit dem ungespannten /\textipa{\u{E}}/ (\textit{ändern}) und dem  gespannten /\textipa{E}/ (\textit{Ähre}).
...
Wie oben bereits erwähnt  finden wir im Bereich der langen Silben bei den offenen Einsilblern wie \textipa{[kni:]}, \textipa{[Su:]} und \textipa{[Ke:]} immer eine Dehnungsschreibung (\textit{Knie}, \textit{Schuh}, \textit{Reh}).
...
Die Annahme des Silbengelenks erlaubt es, alle scheinbaren kurzen offenen und damit einmorigen Silben im deutschen Kernwortschatz als geschlossen mit Silbengelenk und damit zweimorig zu analysieren.
\index{Silbengelenk}
}
...
\begin{figure}
  ...
  \Tree{
    ...
    \K{Anfangsrand}\B{d} && \K{Kern}\B{d} && \K{Enrand}\B{dr} && \K{Anfangsrand}\B{dl} && \K{Kern}\B{d} \\
...
Unter der Annahme des Silbengelenks sind alle Silben (außer unbetonbaren Silben, also Schwa-Silben) entweder zweimorig (entweder langer Vokal und leerer Endrand oder kurzer Vokal und simplexer Endrand) oder dreimorig (entweder langer Vokal und simplexer Endrand oder kurzer Vokal und duplexer Endrand).
In scheinbar kurzen offenen Erstsilben von Mehrsilblern wird Zweimorigkeit dadurch hergestellt, dass der Konsonant im Anfangsrand der Folgesilbe dank Gelenk-Status zum Silbengewicht der Erstsilbe gerechnet wird.}
