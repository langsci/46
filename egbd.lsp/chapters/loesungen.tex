...
\begin{enumerate}\Lf
  ...
  \item Es gibt kein Minimalpaar.
    Die einzigen in Frage kommenden Segmente wären /\textipa{\c{c}}/ und /\textipa{X}/.
    Diese sind komplementär verteilt.
  \item Schicht (/\textipa{\c{c}}/), schickt (/\textipa{k}/)
  \item fahren (/\textipa{f}/), wahren (/\textipa{v}/)
  \item Post (/\textipa{p}/), Kost (/\textipa{k}/)
  \item Wall, Wal
  \item Mutter (/\textipa{U}/), Mütter (/\textipa{Y}/)
\end{enumerate}
...
\begin{center}
  \resizebox{\textwidth}{!}{
    \begin{tabular}[h!]{lccccc}
      \lsptoprule
      \textbf{Silbe} & \textbf{Nukleus} & \textbf{Sonoritätsk.} & \textbf{Onset-Max.} & \textbf{Plateau} & \textbf{Einsilbler} \\
      \midrule
      Strumpf & $\times$ & -- & $\times$ & $\times$ & $\times$ \\
      \hline
      wring & $\times$ & $\times$ & $\times$ &  $\times$& $\times$ \\
      en & $\times$ & $\times$ & -- & $\times$ & -- \\
      \hline
      wink & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      te & $\times$ & $\times$ & -- & $\times$ & -- \\
      \hline
      Quark & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      spei & $\times$ & -- & -- & $\times$ & $\times$ \\
      se & $\times$ & $\times$ & $\times$ & $\times$ & -- \\
      \hline
      Le & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      ser & $\times$ & $\times$ & $\times$ & $\times$ & -- \\
      \hline
      Le & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      se & $\times$ & $\times$ & $\times$ & $\times$ & -- \\
      rin & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      \hline
      zu & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      setz & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      lich & $\times$ & $\times$ & -- & $\times$ & $\times$ \\
      \hline
      zu & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      setz & $\times$ & $\times$ & $\times$ & $\times$ & $\times$ \\
      li & $\times$ & $\times$ & -- & $\times$ & -- \\
      che & $\times$ & $\times$ & $\times$ & $\times$ & -- \\
      \hline
      Ha & $\times$ & $\times$ & $\times$ & $\times$ & -- \\
...
Die Sonoritätskontur ist hier nicht in Ordnung, denn /s/ steht als Frikativ zwischen /b/ bzw.\ /g/ und /t/, also zwischen zwei Plosiven.
Wir können das hier nur als Ausnahme zur Regularität hinnehmen.
Immerhin betrifft die Ausnahme wieder /s/-Segmente, was den Schaden für die Theorie eingrenzt.
...
\Loesungen{sec:morphologie}

\Loesung{u61}

Diese Übung hat unter anderen das Ziel, darauf hinzuweisen, dass Kasus oft keine eindeutig festzulegende Semantik haben.
...
\begin{center}
  \resizebox{\textwidth}{!}{
    \begin{tabular}{lllll}
      \lsptoprule
      \textbf{Analyse} & \textbf{Kopf} & \textbf{Typus} & \textbf{produktiv} & \textbf{Ausgangswort} \\
      \midrule
      (Wesen-s.zug)-s.analyse & Analyse & Rek./Det. & prod. & simplex \\
      Einschub.öffnung & Öffnung & Rek./Det. & prod. & Suffix \textit{:ung} \\
      Ess.tisch & Tisch & Determinativk. & transp. & simplex \\
      (Räder.werk)-s.reparatur & Reparatur & Rektionsk. & prod. & Fremdsuffix \textit{:tur} ? \\
      Einschieb-e.öffnung & Öffnung & Determinativk. & prod. & Suffix \textit{:ung}\\
      Groß.rechner & Rechner & Determinativk. & transp. & Suffix \textit{:er} \\
      (Bank.note)-n.fälschung & Fälschung & Rek./Det. & prod. & Suffix \textit{:ung} \\
      ((Berg.bau).wissenschaft)-s.studium & Studium & Determinativk. & prod. & Fremdsuffix \textit{:um} ? \\
      Anschlag-s.vereitelung & Vereitelung & Rektionsk. & prod. & Suffix \textit{:ung} \\
      &&&& Verbpräfix \textit{ver:} \\
      Bio.laden & Laden & Determinativk. & prod. & simplex \\
      Kind-er.garten & Garten & Determinativk. & transp. & simplex \\
      Mit.bewohner & Bewohner & Determinativk. & transp. & Suffix \textit{:er} \\
      &&&& Verbpräfix \textit{be:} \\
      (Absicht-s.erklärung)-s.verlesung & Verlesung & Rektionsk. & prod. & Suffix \textit{:ung} \\
      &&&& Verbpräfix \textit{ver:} \\
      Monat-s.planung & Planung & Rek./Det. & prod. & Suffix \textit{:ung} \\
      feuer.rot & rot & Determinativk. & transp. & simplex \\
      (Not.lauf).programm & Programm & Determinativk. & prod. & simplex \\
      \lspbottomrule
    \end{tabular}
  }
\end{center}

Bei \textit{Wesenszugsanalyse} und den anderen, für die Rek./Det.\ angegeben ist, gibt es jeweils eine Lesart as Determinativkompositum und eine als Rektionskompositum.
...
Die Unterscheidung transparent/produktiv ist nicht immer eindeutig.
Die Tabelle soll hier nur eine Diskussionsgrundlage liefern.
...
\begin{center}
  \resizebox{\textwidth}{!}{
    \begin{tabular}{llllll}
      \lsptoprule
      \textbf{Analyse} & \textbf{Klasse Ausgangs-/Zielwort} & \textbf{Typ} & \textbf{Umlaut} & \textbf{produktiv} \\
        \midrule
        (ver:käuf):lich & V > V > Adj & Deriv., Deriv. & ja (\~-lich) & transp., prod. \\
        unter:wander & V > V & Deriv. & nein & transp. \\
        alternativ:los & Subst > Adj & Deriv. & nein & prod. \\
        Lauf & V > Subst & Stammkonv. & nein & transp. \\
        auf\v{:}steig & V > V & Deriv. & nein & prod. \\
        Ge:bell & V > Subst & Deriv. & nein & prod. \\
        be:schließ & V > V & Deriv. & nein & transp. \\
        be:gegn & V > V & Deriv. & nein & transp. \\
        Röhr:chen & Subst > Subst & Deriv. & ja (Rohr) & prod. \\
         &&& nein (Röhre) & \\
        Schlingern & V > Subst & Wfkonv. & nein & prod. \\
        Ge:ruder & V > Subst & Deriv. & nein & prod. \\
        Über:(zock:er) & V > Subst > Subst & Deriv., Deriv. & nein & prod., prod.\\
        Ge:brüder & Subst > Subst & Deriv. & ja? & transp.? \\
        Münd:el & Subst > Subst & Deriv. & ja? & transp.? \\
        schweig:sam & V > Adj & Deriv. & nein & prod.\\
        \lspbottomrule
    \end{tabular}
  }
\end{center}
...
Es gibt kein Wort \textit{\Ast Ge:bruder}, und \textit{Ge\~-} ist hier ggf.\ eine Art unregelmäßiger Pluralbildung, womit der Status als Wortbildung fraglich wäre.
...
Genaueres zu diesen Konstruktionen findet sich in Kapitel \ref{sec:praedikate}.
  ...
  \item\label{it:99001}
    \resizebox{0.9\textwidth}{!}{
      \Tree{
        ...
        & \K{KP\ORiii}\TRi[-2] && \K{\textbf{V\ORi}}\B{d} && \K{NP}\TRi[-7] & \K{\Tii} & \K{Ptkl}\B{d} & \K{\Ti} & \\
