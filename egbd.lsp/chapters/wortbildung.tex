...
Das Kompositum erhält seine grammatischen und semantischen Merkmale auf produktive oder zumindest transparente Weise von den beiden Gliedern.
\index{Komposition}
}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex[\#]{Gebrauchtmöbel sind gebraucht.}
...
Die Gruppe aus (\ref{ex:detkomp}), also \textit{Schul.heft}, \textit{Staats.finanzen}, \textit{Gebraucht.möbel} usw. werden \textit{Determinativkomposita} genannt, weil der Nicht-Kopf den Kopf semantisch näher bestimmt (\textit{determiniert}), aber eben keine Rektionsbeziehung gegeben ist.
...
Man erkennt an diesen Beispielen, dass der Beitrag des Affixes zur Bedeutung des Zielwortes meistens sehr eindeutig ist.
...
Danach erfolgt ein Überblick über Derivationsaffixe mit Wortklassenwechsel und abschließend eine Betrachtung von Einschränkungen bei der rekursiven Derivation durch Suffigierung.
...
Diese drei Eigenschaften sind in (\ref{ex:vpart}) und (\ref{ex:vpre}) bebeispielt, wobei als Trennzeichen für die Verbpartikeln \v: verwendet wird.
...
\begin{exe}
  ...
  \begin{xlist}
  	\ex{\label{ex:wb983460a} Nadezhda klagt über die schlechte Qualität der Hantel.}
  	\ex{\label{ex:wb983460b} Nadezhda beklagt die schlechte Qualität der Hantel.}
  \end{xlist}
...
Der wesentliche Unterschied ist hier, dass die von \textit{brechen} regierte Präposition formal dem Präfix entspricht.
...
\footnote{Ob es eine fakultative Ergänzung oder eine Angabe ist, ist hier nicht ausschlaggebend.}
Das Verb \textit{auf\v{:}schreiben} wird mit einer gleichlautenden Partikel \textit{auf\v{:}} gebildet und tilgt damit gleichsam die präpositionale Ergänzung.
Das systematische Bild wird getrübt durch Fälle wie (\ref{ex:wb983462c}), in denen Partikel und Präposition auftreten.
Solche Sätze sind vielleicht stilistisch nicht herausragend, aber nicht eindeutig ungrammatisch.
...
Dieser Bereich der Valenzänderungen durch Präfixe und Tilgung von Ergänzungen durch Partikeln ist komplex, und es gibt Unterschiede zwischen Unterklassen der Präfixe und Partikeln, individuelle Unterschiede, und diverse nicht oder eingeschränkt produktive Fälle.
Man kann nicht erwarten, mit den genannten drei Mustern jedem Präfix- oder Partikelverb beizukommen.
Vollständigere Grammatiken wie \citet{Eisenberg1} bieten eine bessere Gesamtschau.

\subsection{Derivation mit Wortklassenwechsel und Mehrfachsuffigierung}
...
Die Tabelle deutet durch die relative Anzahl der genannten Affixe an, dass Derivations-Affixe häufig Substantive, seltener Adjektive und noch seltener Verben bilden.
...
Weiter oben (Satz~\ref{satz:reknrek}, S.~\pageref{satz:reknrek}) wurde nun festgestellt, dass Wortbildung rekursiv sei.
...
Die genaue Diskussion der Flexion ist hier der Wortbildung unter anderem deshalb nachgeordnet, weil es so möglich wird, ggf.\ zu diskutieren, ob bestimmte Bildungen tatsächlich Flexion oder doch eher Wortbildung sind (\zB die Komparation, s.\ Abschnitt~\ref{sec:kompform}).
