...
Die Auflösung erfolgt jetzt, indem die nicht umgelauteten und die zugehörigen umgelauteten einfachen Vokale im (phonologischen) Vokalviereck dargestellt werden (vgl.\ Abbildung~\ref{fig:vokalviereckmitgespannt} auf S.~\pageref{fig:vokalviereckmitgespannt}).\index{Vokalviereck}
...
\Definition{Umlaut}{
\label{def:umlaut}
Der Umlaut ist ein systematischer, morphologisch bedingter Unterschied der Vokalqualität in Wortstämmen (Frontierung).
...
\Definition{Ablaut}{
\label{def:ablaut}
Der Ablaut ist ein systematischer, morphologisch bedingter Unterschied der Vokalqualität in Wortstämmen.
...
Dabei sind \textit{Morphe} die zentralen Bausteine, aus denen Wortformen aufgebaut werden, und die \textit{Markierungsfunktionen} haben.
Der \textit{Stamm} markiert die Zugehörigkeit zu einem bestimmten lexikalischen Wort.
}
...
\Zusammenfassung{
\textit{Affixe} sind unselbständige Markierungen nicht-lexikalischer Merkmale.
Durch mehrfache \textit{Affigierung} ergeben sich \textit{hierarchische} Strukturen.
}
...
\Satz{Affigierende Wortbildung und Flexion}{
  \label{satz:wobiflexappl}\index{Affigierung}
Flexionsprozesse sind prototypisch affigierend, aber i.\,d.\,R.\ auf alle Wörter einer Wortklasse anwendbar.
...
Bei der \textit{Flexion} ändern sich (durch Formänderungen und Affigierung) Merkmale in \textit{Paradigmen}.
Bei der \textit{Wortbildung} ändern sich ansonsten \textit{statische Merkmale}, es entstehen also neue Wörter.
}
...
Eine \textit{phonologische Konditionierung} liegt in (\ref{ex:morph9239}) vor.\index{Konditionierung!phonologisch}
...
Eine \textit{grammatische Konditionierung} liegt in (\ref{ex:morph9240}) vor.\index{Konditionierung!grammatisch}
...
Ein Beispiel für eine \textit{lexikalische Konditionierung} findet sich in (\ref{ex:morph9241}).\index{Konditionierung!lexikalisch}
