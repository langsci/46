...
\Definition{Syntax}{
\label{def:syntax}
Eine Syntax formuliert die Generalisierungen, die genau die Sätze einer natürlichen Sprache (nicht mehr oder weniger oder andere Sätze) beschreiben.
...
Hier kann eine ebenfalls rekursive Koordinationstruktur beliebig fortgesetzt werden.\label{abs:syntaxrekursion}
...
Wenn eine Syntax eine gegebene Folge von Wörtern auf Basis ihrer Generalisierungen als Satz beschreiben kann, gilt der Satz relativ zu dieser Syntax als \textit{grammatisch}.
...
Sätze in der Grammatik als Folgen von Wörtern zu beschreiben ist nicht zielführend, weil es viel zu viele verschiedene Arten von Wortfolgen gibt, die grammatisch sind.
Man beschreibt zunächst die Struktur \textit{kleinerer Konstituenten} (NPs, PPs usw.), aus denen dann \textit{größere Konstituenten} und schließlich \textit{Sätze} aufgebaut werden.
}
...
\begin{exe}
  \ex\label{ex:syn0815}
  \begin{xlist}
  	\ex{\label{ex:syn0815a} Gestern hat [Pandelela] [beim Turmspringen] [die Goldmedaille] gewonnen.}
  	\ex{\label{ex:syn0815b} Gestern hat [beim Turmspringen] [Pandelela] [die Goldmedaille] gewonnen.}
  	\ex{\label{ex:syn0815c} Gestern hat [beim Turmspringen] [die Goldmedaille] [Pandelela] gewonnen.}
  \end{xlist}
\end{exe}
...
In der ersten Verbphrase findet dabei eine Ellipse (Weglassung) des Verbs statt, um seine unnötige Wiederholung zu vermeiden.\index{Ellipse}
...
\Zusammenfassung{
  Die \textit{Konstituententests} sind eine Heuristik, mit deren Hilfe man sich einer zielführenden Konstituentenanalyse annähern kann.
Sie stellen keine notwendige oder hinreichende empirische Grundlage für die Syntax dar.
Selbständig bewegbare und \textit{vorfeldfähige} Konstituenten werden \textit{Satzglieder} genannt.
Eine Folge von Wörtern kann \textit{strukturell ambig} sein, also mehr als eine angemessene syntaktische Analyse haben.
}
...
\label{def:dependenz}
Eine Konstituente A ist von einer Konstituente B im selben Satz abhängig (dependent), wenn die Anwesenheit von B eine Bedingung für die Anwesenheit und\slash oder die Form von A ist.
...
Der Begriff des \textit{Kopfes} wird zusammen mit dem der \textit{Phrase} definiert, da Phrasen einen Kopf haben.
...
\Definition{Funktionswort}{%
\label{def:funktionswort}\index{Funktionswort}\index{Partikel}\index{Artikelwort}\index{Konjunktion}%
Ein Funktionswort hat keinen Kopfstatus, bildet keine Phrasen und ist nicht vorfeldfähig.
Es ist damit \textit{abhängig} in dem Sinn, dass es nicht alleine auftreten kann und keine Rektion bzw.\ Valenz hat.
...
\Zusammenfassung{
  Eine Phrase hat typischerweise einen \textit{Kopf}, der ihre wesentlichen Merkmale bestimmt, die \textit{Kopf-Merkmale}.
Innerhalb einer Phrase sind alle Konstituenten direkt oder indirekt vom Kopf abhängig (\textit{dependent}).
Funktionswörter sind prinzipiell abhängige Wörter, die keine eigenen Phrasen bilden.
Syntaxbäume müssen gewissen Regeln entsprechen, \zB dass sie nur einen \textit{Wurzelknoten} haben.
}
