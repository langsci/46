...
\Zusammenfassung{
  Die verschiedenen \textit{nominalen Kategorien} (bzw.\ Merkmale) sind teilweise semantisch\slash pragmatisch motiviert (\zB Numerus), teilweise aber eher strukturell bzw.\ rein grammatisch (\zB Kasus).
  Kasus lassen sich in einer \textit{Hierarchie} anordnen, je nachdem wie stark sie einen eigenständigen semantischen Beitrag leisten (\textit{oblik}) bzw.\ nur Rektionsanforderungen erfüllen (\textit{strukturell}).
}
...
Ein Affix der Form \textit{\char`~e} löst Umlaut aus, auch wenn es ansonsten vollständig getilgt wird.
}
...
Die Flexion der schwachen Substantive (\textit{en}"=Substantive) wie \textit{Bär} oder \textit{Linguist} folgt in jeder Hinsicht einem eigenen Schema.
}
...
\Zusammenfassung{
Artikel und Pronomina müssen lexikalisch genau dann unterschieden werden, wenn ihre Formen nicht immer übereinstimmen, \zB \textit{unser Haus} und \textit{unseres}.
Die \textit{Indefinitartikel} (\zB \textit{kein}) und \textit{Possessivartikel} (\zB \textit{mein}) haben im Nominativ Maskulinum und Neutrum sowie im Akkusativ Neutrum keine Endungen.
}
...
\Zusammenfassung{
  Das \textit{attributive Adjektiv} übernimmt entweder die Flexionssuffixe der Pronomina (stark), oder es hat einen reduzierten eigenen Satz von Suffixen (schwach) mit \textit{-e} und \textit{-en}.
