...
Zuvor muss allerdings der Begriff der Nominalphrase eingeführt werden, den wir im Rahmen der Flexion als Hilfsbegriff benötigen, und der in Abschnitt~\ref{sec:ngr} genauer und ausführlicher eingeführt wird.

\index{Nominalphrase}

\Definition{Nominalphrase (vorläufig)}{
\label{def:vollstaendigesnominal}
Eine Nominalphrase (NP) ist eine zusammenstehende Gruppe aus einem Substantiv, eventuell davorstehenden Adjektiven und einem eventuell davorstehenden Artikelwort.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:flex8882a} [Spielerinnen] habe ich keine gesehen.}
    \ex{\label{ex:flex8882b} [Hervorragende Spielerinnen] hatte der FCR schon immer.}
    \ex{\label{ex:flex8882c} [Eine hervorragende Spielerin] schoß das entscheidende Tor für den FCR.}
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Die Trainerin beobachtet Spielerinnen.}
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex[]{Die Trainerin beobachtet [einen guten Pass].}
    \ex[*]{Die Trainerin beobachtet [einen guten Pässe].}
  \end{xlist}
  \ex \label{ex:flex9112}
  \begin{xlist}
    \ex[]{Die Trainerin beobachtet [einige gute Pässe].}
    \ex[*]{Die Trainerin beobachtet [einige gute Pass].}
  \end{xlist}
\end{exe}
...
Bestimmte Substantive treten aus semantischen Gründen oder aus Idiosynkrasie (wortspezifische Unregelmäßigkeit) nur im Singular oder im Plural auf.
Man spricht von sogenannten Singulariatantum oder Pluraliatantum, vgl.\ (\ref{ex:flex6662}) und (\ref{ex:flex6663}).%
\footnote{Im Singular lauten diese Wörter Singularetantum bzw.\ Pluraletantum.}
...
Auch wenn \textsc{Numerus} ein semantisch motiviertes Merkmal ist, so zeigt sich doch an den in diesem Abschnitt zitierten Beispielen, dass er diverse strukturelle Regularitäten mit sich bringt, wie \zB die Kongruenz zwischen verschiedenen zusammengehörigen Merkmalen.
...
\BVertiefung{Numeruskongruenz und Koordination}{Im Fall einer sog.\ Koordinationsstruktur mit Konjunktionen wie \textit{und} oder \textit{oder} (vgl.\ Genaueres in Abschnitt~\ref{sec:koor}) kongruieren die mit der Konjunktion verbundenen NPs in ihrem Numerus nicht miteinander.
...
Bezüglich Kasus herrscht dennoch Übereinstimmung, ob man diese nun theoretisch als Kongruenz analysiert oder nicht.
...
\subsection{Kasus und Kasushierarchie}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Wer oder was ging ins Aus? -- Der Ball. $\rightarrow$ Wer-Fall\slash Nominativ}
  \end{xlist}
  \ex \label{ex:flex2773}
  \begin{xlist}
    \ex{Der Ball kollidierte mit dem Pfosten.}
    \ex{Der Ball kollidierte mit wem oder was? -- Dem Pfosten. $\rightarrow$ Wem-Fall\slash Dativ}
  \end{xlist}
\end{exe}
...
Die Katastrophe in (\ref{ex:flex4444}) wurde von einem niedersächsischen Deutschlehrer in der Sekundarstufe I (im Jahr 2005) vertreten.
Der in der Akkusativ-NP enthaltene Genitiv wird nicht korrekt erkannt, weil irgendwie diffus über Bedeutung nachgedacht wird, und weil keine ordentliche Konstituentenanalyse vor der Kasusbestimmung durchgeführt wird.
...
Das Merkmal \textsc{Kasus} kann nicht über solche einfachen Fragen zielsicher ermittelt werden, weil seine Werte sehr oft durch Rektion gesetzt werden (vgl.\ ausführlich Abschnitt~\ref{sec:valenzundkoepfe}).
...
\Definition{Deiktische Ausdrücke}{
\label{def:deixis}
...
\Definition{Anaphern}{
\label{def:anaphern}
Anaphern sind Audrücke, die die Bedeutung eines im Satz oder Text vorangehenden Ausdrucks (Antezedens) wieder aufnehmen.
Anapher und Antezendens sind korreferent (Gleiches bezeichnend).
}
...
Zwei Ausdrücke mit demselben Index sind korreferent und werden manchmal auch koindiziert genannt.
...
\begin{figure}[!h]
  \resizebox{\textwidth}{!}{
    \Treek[1]{3}{
      ...
      \K{\textbf{schwach} (S1)} && \K{\textbf{stark} (S2)}\Bdash{dl}\Bdash{d}\Bdash{dr} && \K{\textbf{gemischt} (S3)} && \K{\textbf{\textit{s}-Flexion} (S5)} && \K{S4}\Bdash{dl}\Bdash{dr}\\
      & \K{Plural}\Below{\textit{\~-e} (S2a)} & \K{Plural}\Below{\textit{-e} (S2b)} & \K{Plural}\Below{\textit{\~-er} (S2c)} &&&& \K{Plural}\Below{\textit{-en} (S4a)} && \K{Plural}\Below{\~-e (S4b)} \\
    }
  }
...
Die Unterscheidung in stark, schwach und gemischt betrifft nur die Maskulina und Neutra, dabei aber nicht die \textit{s}-Flexion.
...
Der Entscheidungsbaum in Abbildung~\ref{fig:substklassentsch} zeigt, wie die Haupt-Flexionsklasse eines Substantivs ermittelt werden kann.

\begin{figure}[!h]
  \centering
  \Treek[2]{4}{
    && \K{Genus?}\QQw\DLqQ_{\textnormal{Mask}}^{\textnormal{Neut}}\DRq^{\textnormal{Fem}}\\
    & \K{Gen Sg?}\QQw\DLq_{\textnormal{\textit{-(e)n}}}\DRqQ^{\textnormal{\textit{-(e)s}}} && \K{\textbf{Fem}}\Below{(S4)} \\
    \K{\textbf{schwach}}\Below{(S1)} && \K{Nom Pl?}\QQw\DRq^{\textnormal{\textit{-(e)n}}}\DLq_{\textnormal{\textit{\~-e\slash-e}}}^{\textnormal{\textit{\~-er}}} && \\
...
\begin{table}[!h]
  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}{lp{0mm}lp{1mm}lllp{1mm}lp{1mm}llp{1mm}l}
      \lsptoprule
      \textbf{Klasse} && Mask schwach && \multicolumn{3}{l}{Mask\slash Neut stark} && Mask\slash Neut gem. && \multicolumn{2}{l}{Fem} && \textit{s}-Flexion \\
      \textbf{Nummer} && S1 && S2a & S2b & S2c && S3 && S4a & S4b && S5 \\
      \midrule
      \textbf{Markierung} && \textbf{-en} && \textbf{\~-e} & \textbf{-e} & \textbf{\~-er} && \textbf{-en} && \textbf{-en} & \textbf{\~-e} && \textbf{-s} \\
...
Wenn wir jetzt Tabelle~\ref{tab:tradflexsubst} als Tabelle~\ref{tab:tradflexsubst-r} wiederholen und die Plural-Affixe zusätzlich abtrennen, können wir uns Gedanken darüber machen, auf welche Weise das verbleibende Affix-Material Kasus anzeigt.
...
\BVertiefung{Eigennamen und \textit{s}-Flexion}{
\index{Eigenname}
Bei der \textit{s}-Flexion, besonders auffällig aber bei Verwandschaftsbezeichnungen wie \textit{Oma} und \textit{Opa}, ist die Flexionsklassenzugehörigkeit differenziert zu betrachten.
...
In (\ref{ex:flex7289a}) flektiert \textit{Oma} als Femininum der \textit{s}-Flexion ohne Genitiv-Suffix (wie alle Feminina), der Plural \textit{Oma-s} in (\ref{ex:flex7289b}) weist \textit{Oma} aber durchaus als \textit{s}-Substantiv aus.
Das Maskulinum der \textit{s}-Flexion \textit{Opa} hat das \textit{-s} des Genitivs (\ref{ex:flex7290a}) und das \textit{-s} des Plurals (\ref{ex:flex7290b}).

Warum aber ist der Genitiv sowohl in (\ref{ex:flex7290c}) als auch (\ref{ex:flex7289c}) mit \textit{-s} markiert?
Wenn \textit{Oma} und \textit{Opa} Substantive der \textit{s}-Flexion sind, sollte nur \textit{Opa} im Genitiv ein \textit{-s} suffigiert werden.
...
Eigennamen von Personen flektieren im Prinzip (unabhängig von Geschlecht oder Gender der Person) nach der \textit{s}-Flexion und nehmen dabei im Genitiv das Suffix \textit{-s}, vgl.\ (\ref{ex:flex7291a}).
...
Auch damit wird das Paradigma leider nicht vollständig regularisiert, weil der Nominativ nie zu \Ast(\textit{ein}) \textit{Menschen} angeglichen wird.
Ansonsten wäre das Muster dasselbe wie bei \textit{Kuchen}.
Es existiert eine weitere Variante.
Diese Variante ist der vollständige Übergang des Worts in die gemischte Flexion, also (\ref{ex:flex2202}), vgl.\ (\ref{ex:flex2203}).
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{(den) Mensch}
    \ex{(dem) Mensch}
    \ex{(des) Mensch-s / Mensch-es}
  \end{xlist}
  \ex\label{ex:flex2203}{Aber ist der Respekt genügend für das Bewusstsein des Mensches? (DECOW2012-00:358152715)}
\end{exe}
...
\begin{enumerate}\Lf
  ...
  \item\label{it:pluralklass} alle anderen Substantivklassen lediglich nach ihrer Pluralbildung unterschieden werden, wobei \textit{\~-e} (oder \textit{-e}) prototypisch für die Maskulina und Neutra ist und \textit{-en} prototypisch für die Feminina ist,\index{Numerus}
...
\begin{figure}[h!]
  ...
  \Treek[3]{4}{
    ...
    & \K{\textit{\~-er}}\Below{(nur M/N)} & \K{\textit{\~-e}\slash\textit{-e}}\Below{(M/N-Prototyp)} & \K{\textit{-en}}\Below{(F-Prototyp)} & \K{\textit{-s}} \\
  }
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Wie ist es mit Texten? -- Er schreibt einen.}
  \end{xlist}
\end{exe}

Daneben können die Pronomina der dritten Person in vielen Fällen auch als Artikel verwendet werden, und man spricht als Sammelbezeichnung dann vom \textit{Artikelwort}.\index{Artikelwort}
...
Die in Kapitel~\ref{sec:wortklassen} offengelassene Unterklassifikation der Artikel und Pronomina kann nun durchgeführt werden.
...
So müsste in (\ref{ex:flex4233}) neben \textit{sein-em} und \textit{sein-e} eine andere Einheit vorkommen, die [\textsc{Person}: \textit{3}, \textsc{Numerus}: \textit{sg}, \textsc{Genus}: \textit{mask}] sein müsste, um im Falle von \textit{sein} von grammatischer Kongruenz sprechen zu können.
}
...
\Satz{Flexion der Pronomina\slash Artikel und ihre Wortklassen}{
\label{satz:pronflexwklassmap}
\vspace{-0.8cm}
\begin{enumerate}\Lf
  \item Reine Artikel sind der Definitartikel und die Indefinitartikel sowie die Possessivartikel%
    \footnote{\label{ft:nomina0815}Stämme: \textit{der}\slash\textit{die}\slash\textit{das}, \textit{ein}, \textit{kein}, \textit{mein}, \textit{dein}, \textit{sein}, \textit{ihr}, \textit{euer}, \textit{unser}}
  \item Reine Pronomina sind Pronomina, für die ein gleichlautender Stamm eines Indefinit-\slash Possessivartikels existiert.\footref{ft:nomina0815}
...
\begin{figure}[h!]
  ...
  \Treek[1.5]{3}{
    ...
    & \K{andere}\B{dl}\B{dr} && \K{Indefinit-\slash Possessivartikel}\Below{(\textit{kein}, \textit{mein} usw.)} \\
    \K{normale Pronomina}\Below{(\textit{jener}, \textit{meiner} usw.)} && \K{Definita}\B{dl}\B{dr} \\
    & \K{Definitpronomen}\Below{(\textit{der}, \textit{dessen} usw.)} && \K{Definitartikel}\Below{(\textit{der}, \textit{des} usw.)}
  }
...
\subsection{Flexion der Pronomina und definiten Artikel}
...
\subsection{Flexion der indefiniten Artikel und Possessivartikel}
...
Wir können Adjektive also danach klassifizieren, ob sie nur attributiv oder auch prädikativ verwendet werden können.
...
\subsubsection{Die Stärkeparadigmen}
...
Bereits in Abschnitt~\ref{sec:prondefart} wurde herausgestellt, dass die Formendifferenzierung bei den Pronomina (und dem definiten Artikel) relativ gut ist.
...
\label{satz:adjflex}
Wenn der (definite\slash pronominale) Artikel fehlt, wird die formale Differenzierung von Kasus und Numerus vom Adjektiv übernommen, wozu beim Adjektiv auch überwiegend dieselben Suffixe wie beim Pronomen verwendet werden.
Beim indefiniten Artikel, der die sogenannte gemischte Flexion auslöst, übernimmt das Adjektiv die Markierung aber nur in genau den drei Formen, in denen dem indefiniten Artikel (warum auch immer) die Suffixe fehlen.
Der gemischte Flexionstyp ist damit kein eigener Flexionstyp, sondern kann über allgemeine Regularitäten abgeleitet werden.
}
...
\Satz{Monoflexion}{
\label{satz:monoflex}
Das Deutsche hat die Tendenz, dass Kasus und Numerus innerhalb der NP im Idealfall nur höchstens einmal eindeutig markiert werden.
\index{Monoflexion}
}
...
\subsubsection{Markierung in der adjektivalen (schwachen) Flexion}

\label{sec:schwacheadj}

\index{Adjektiv!adjektival}

Wie ist nun die eigentliche adjektivale (schwache) Flexion bezüglich der Markierungsfunktionen der Suffixe zu bewerten?
...
Damit lässt sich die Flexion der Adjektive abschließend mit einer einzigen Fallentscheidung, einer Regularität und zwei Ausnahmen zusammenfassen, Abbildung~\ref{fig:adjregeln}.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Die Ideen der Aufklärung sind am schönsten von allen.}
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:flex-2560a} Ein schärf-er-es Chili als meins gibt es nicht.}
    \ex{\label{ex:flex-2560b} Das schärf-st-e Chili von allen ist wie immer meins.}
    \ex{\label{ex:flex-2560c} Mein Chili ist schärf-er als deins.}
    \ex{\label{ex:flex-2560d} Mein Chili ist am schärf-st-en von allen.}
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{Mein Chili ist scharf.}
    \ex{Mein Chili ist schärfer [als andere].}
    \ex{Mein Chili ist am schärfsten [von allen].}
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{Mein Chili ist schärfer, wenn man es mit den bisher probierten Chilis vergleicht.}
    \ex{Mein Chili ist im internationalen Vergleich am schärfsten.}
  \end{xlist}
\end{exe}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:flex7778a} Mein Chili ist genauso scharf wie deins.}
    \ex{\label{ex:flex7778b} Ein ähnlich scharfes Chili wie dieses habe ich mal in einem veganen Restaurant in Dallas gegessen.}
  \end{xlist}
\end{exe}
