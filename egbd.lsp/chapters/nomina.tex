...
Semantische Motivation bedeutet hier, dass es von den zu beschreibenden Sachverhalten in der Welt und nicht von grammatischen Bedingungen abhängt, ob eine Singular- oder eine Pluralform gewählt wird.
...
Bestimmte Substantive treten aus semantischen Gründen oder aus \textit{Idiosynkrasie} (wortspezifische Unregelmäßigkeit) nur im Singular oder im Plural auf.
...
Sprecher verweisen mit diesen Pronomina auf bestimmte in der Sprechsituation anwesende Dinge, und man spricht von \textit{deiktischen} Ausdrücken.
...
\label{def:anaphern}
Anaphern sind Audrücke, die die Bedeutung eines im Satz oder Text vorangehenden Ausdrucks (des Antezedens) wieder aufnehmen.
...
\textbf{?? TODO 2a und 2b besser diskutieren bzgl. Umlaut bei Fem und MN}
...
Die Unterscheidung in stark, schwach und gemischt betrifft nur die Maskulina und Neutra, dabei aber nicht die s-Flexion.
...
Der Entscheidungsbaum in Abbildung~\ref{fig:substklassentsch} zeigt, wie die primäre Flexionsklasse eines Substantivs ermittelt werden kann.
...
\begin{figure}
  ...
  \caption{Entscheidungsbaum für die Flexionsklassenzugehörigkeit (traditionelle Terminologie)}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    Ahn, Bauer, Bube, Bürge, Bursche, Depp, Erbe, Fürst, Gatte, Gehilfe, Genosse, Graf, Heide, Held, Herr, Hirte, Junge, Knabe, Mensch, Nachbar, Narr, Neffe, Riese, Schurke, Sklave, Steinmetz, Tor, Typ, Untertan, Waise, Zeuge, Zwerg
    \ex\label{ex:flex374311b} Erbwörter, Lebewesen\\
    Affe, Bär, Bulle, Falke, Fink, Hase, Löwe, Ochse, Rabe, Rüde 
    ...
    \ex\label{ex:flex374311d} Lehnwörter mit charakteristischen  Endungen (überwiegend Menschen)\\
    Ignorant, Astronaut, Architekt, Patient, Planet, Paragraf, Linguist, Israelit, Biologe, Astronom, Zyklop, Dramaturg
  \end{xlist}
\end{exe}
...
Der \textit{s}-Plural ist zwar typisch für bestimmte Substantive (vor allem solche, die auf Vollkoval ausgehen), aber von ihrer Formenreihe passt die s-Flexion perfekt in das allgemeine Muster.
...
Einem völlig eigenen Muster folgen die \textit{Personalpronomina} wie \textit{ich}, \textit{du} usw., die wir auch nicht weiter betrachten.\index{Pronomen!Personal--}
...
\subsection{Klassifikation und Verwendung}
...
In (\ref{ex:flex9857}) steht das Adjektiv unflektiert (als reiner Stamm, auch \textit{Kurzform} genannt) außerhalb der NP und tritt immer mit einer Form der sogenannten Kopulaverben (vgl.\ Abschnitt~\ref{sec:kopulakonstruktionen}) wie \textit{sein}, \textit{bleiben} oder \textit{werden} auf.\index{Kopula}
...
\textbf{?? ÄNDERN keine Konversion Adj -> Adv}
...
In ihrer Semantik sind adverbial verwendete Adjektive ähnlich vielseitig wie echte Adverben.
...
Beim indefiniten Artikel, der die sogenannte \textit{gemischte} Flexion auslöst, übernimmt das Adjektiv die Markierung aber nur in genau den drei Formen, in denen dem indefiniten Artikel (warum auch immer) die Suffixe fehlen.
...
Die Regularitäten werden abschließend nach der Diskussion der \textit{adjektivalen} Flexion in Abschnitt~\ref{sec:schwacheadj} zusammengfasst.
...
Neben der rein komparativen Verwendung gibt es die sogenannte elativische Verwendung der Superlativformen.\index{Elativ}
...
\citet[177]{Eisenberg1} spricht wahrscheinlich zu Recht von einer \textit{Scheinfrage}, weil die Kriterien für die Abgrenzung der Flexion und der Wortbildung sowieso oft keine ganz exakten Entscheidungen erlauben.
