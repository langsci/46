...
Wir benutzen hier $\kappa$ stellvertretend für Kategoriensymbole wie N, NP, A usw.
Schema~\ref{str:koor} erlaubt nun die Verbindung von irgendeiner syntaktischen Einheit vom Typ $\kappa$ mit einer anderen Einheit derselben Klasse $\kappa$, wobei wieder eine Einheit der Klasse $\kappa$ herauskommt.%
...
Die Relativsätze (und Adjektive) zählen also auch zu den Attributen, und als \textit{innere Rechtsattribute} bezeichnen wir besonders diejenigen Attribute, die rechts vom nominalen Kopf, aber links von Relativsätzen stehen.
...
\footnote{In Schema~\ref{str:ngr} steht \textit{innere Rechtsattribute} als Sammelbegriff, da die Auflistung aller möglichen Typen von Konstituenten nicht übersichtlich möglich ist.
Außerdem würde eine Auflistung nicht der Tatsache gerecht, dass bestimmte Attribute nur dann stehen können, wenn der Kopf sie auch regiert.
...
\Phrasenschema{Nominalphrase (NP)}{\label{str:ngr}
    ...
    \scalebox{0.95}{\begin{tabular}{|c|c|c|c|c|}
      \cline{1-5}
      ( Art\ \slash\ NP[\textsc{Kas}:\textit{gen}] ) & ( AP )* & \textbf{N} & ( \textit{innere Rechtsattribute} )* & ( RS )* \\
      \cline{1-5}
    \end{tabular}
    }
  }
...
Eine attributive AP besteht aus einem Adjektiv und einem optionalen davor stehenden \textit{Intensivierer} (\zB \textit{sehr} und \textit{überaus}).\index{Intensivierer}
Davor stehen wiederum optional verschiedene Arten von Ergänzungen und Modifizierer (ein Sammelbegriff wie \textit{innere Rechtsattribute} in Schema~\ref{str:ngr} auf S.~\pageref{str:ngr}).
Was genau für Ergänzungen vorkommen, wird (im Sinn der Definition von Valenz und Rektion, s.\ Kapitel~\ref{sec:grundbegriffe}) durch das jeweilige Adjektiv bestimmt.
Es ist notwendig, zwei verschiedene strukturelle Positionen in der AP links vom Kopf anzunehmen, weil Intensivierer wie \textit{sehr} immer direkt vor dem Adjektiv stehen und sich nicht mit den Ergänzungen und Modifizierern mischen.
...
\Phrasenschema{Adjektivphrase (AP), nur attributiv}{\label{str:agr}
    ...
    \begin{tabular}{c|c|c|c|}
      \cline{2-4}
      AP = & (\textit{ Modifizierer}\slash \textit{Ergänzungen} )* & ( \textit{Intensivierer} ) & \textbf{A} \\
      \cline{2-4}
    \end{tabular}
...
Typische Modifizierer, die links von der Intensiviererposition stehen, sind \zB temporale oder lokale PP wie \textit{seit gestern} in \textit{seit gestern berühmt} oder \textit{in Hessen} wie in \textit{in Hessen beliebt}.\index{Modifizierer}
Einige APs mit und ohne Intensivierer und Modifizierer werden in (\ref{ex:adjgrnormal}) bebeispielt.
...
Wie erwähnt sind einige der Elemente in der ersten Position innerhalb der AP regierte Ergänzungen, aber nur wenige Adjektive haben eine besondere Valenz (vgl.\ auch Abschnitt~\ref{sec:adjektivklassifikation}).
...
\Phrasenschema{Präpositionalphrase (PP)}{\label{str:prpgr}
  \begin{tabular}{c|c|c|c|}
    \cline{2-4} 
    PP = & ( \textit{Modifizierer} ) & \textbf{P} & NP \\
    \cline{2-4} 
  \end{tabular}
}
...
\Phrasenschema{Adverbphrase (AdvP)}{\label{str:advgr}
  \begin{tabular}{c|c|c|}
    \cline{2-3}
    AdvP = & ( \textit{Modifizierer} ) & \textbf{Adv} \\
    \cline{2-3} 
  \end{tabular}
}
...
\Phrasenschema{Verbphrase (VP)}{\label{str:vgr}
  \begin{tabular}{c|c|c|}
    \cline{2-3}
    VP = & ( \textit{Ergänzungen}\slash \textit{Angaben} )* & \textbf{V} \\
    \cline{2-3}
  \end{tabular}
}
