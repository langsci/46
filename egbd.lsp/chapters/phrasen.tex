...
\TExt{\TXT{\textbf{V$\mathrm{_{\{j,i\}}}$}\strschemspace:\strschemspace(} & \TXT{\textbf{V$\mathrm{_{j}}$}} & \TXT{)\strschemspace\ } & \TXT{\textbf{V$\mathrm{_i}$}}\COny[-]{ll} } }

Schema~\ref{str:vk} ist \textit{rekursiv} (s.\ Abschnitt~\ref{sec:rekursion}), weil es aus mehreren V wieder ein V erzeugt.\index{Rekursivität}
...
Die Notation V\Sub{\{j,i\}} soll verdeutlichen, dass der Verbkomplex einen oder mehrere V-Köpfe (im Schema den regierenden Kopf V\Sub{i} und den regierten Kopf V\Sub{j}) vereint und selber wie ein Kopf fungiert, der die Merkmale und Werte aller an der Verbkomplexbildung beteiligten V-Köpfe einsammelt.%
