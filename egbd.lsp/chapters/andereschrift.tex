...
Klassische Zweifelsfälle entstehen auch bei der potentiellen Bildung neuer Verbpartikeln aus Adjektiven wie \textit{weichklopfen} oder \textit{schlechtreden} und im Bereich der Adjektive durch Verschmelzung mit einer vorangestellten Partikel wie im Fall von \textit{tiefrot} oder \textit{halbfest}.
...
\footnote{Diese Aussage beruht auf einer heimlich unterstellten Trennung zwischen dem \textit{Bindestrich} (-) und hier gar nicht behandelten \textit{Gedankenstrich} (--).
Typographisch ist die Lage der beiden Striche leicht verschieden.
Außerdem ist der Gedankenstrich zumindest in Druckerzeugnissen länger und hat üblicherweise auch eine feinere Strichstärke.
...
Es wird hier vor allem deshalb selten unterschieden, weil handschriftlich die Unterschiede kaum zuverlässig markiert werden können, handelsübliche Schreibmaschinen für alle drei Zeichen nur eine Type hatten, und in Textverarbeitungen der Gedankenstrich üblicherweise von Anwendern nicht gefunden wird.
Es ist also wahrscheinlich, dass hier für Schreiber ein Zeichen mit mehreren Funktionen vorliegt.
Ich halte aus Gründen der Systematik und Geschmacksgründen an einer Trennung fest.}
...
\Zusammenfassung{
\textit{Spatien} trennen syntaktische Wörter.
Morphosyntaktische \textit{Univerbierung} kann zur Zusammenschreibung führen.
Wortklassen werden i.\,d.\,R.\ in der Schreibung nicht markiert, außer im Fall der \textit{positionsunabhängigen Substantiv- und Eigennamengroßschreibung}.
Ehemalige Substantive, die ihren Status als Substantiv verlieren, können dementsprechend kleingeschrieben werden.
Aus \textit{Buchstaben-Abkürzungen} hervorgegangene Wörter wie \textit{LKW}, die komplett in Majuskeln geschrieben werden, sind \textit{Kurzwörter} mit spezifischen grammatischen Eigenschaften. 
\textit{Klitisierung} wird im Deutschen durch den Apostroph markiert, kann bei voranschreitender Konventionalisierung aber auch zu einer Zusammenschreibung führen.
\textit{Konstantschreibungen} sichern die Wiedererkennbarkeit von Stämmen bei Flexions- und Wortbildungsprozessen -- eventuell aber eher als historisches Prinzip.
Wir finden sie vor allem in Form der Schärfungsschreibungen im Einsilbler.
}
...
Weil das Komma genauso wie Konjunktionen alle Arten von Konstituente koordinieren kann, sind Alternativschreibungen wie in (\ref{ex:msschr0018}) möglich.
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:msschr7772c} Mach, dass du raus kommst!}
  \end{xlist}
\end{exe}
...
Das \textit{Nebensatzkomma} markiert Konstruktionen, die prototypisch eine geschlossene VP enthalten und innerhalb ihres Matrixsatzes Satzgliedstatus haben (s.\ Abschnitt~\ref{sec:satzglied} zum Satzglied).}
...
Ausnahmen können durch \textit{parenthetische Phrasen} gebildet werden (Gedankenstrich, Klammer, Komma.)
Unabhängige Sätze werden durch \textit{Satzschlusszeichen} und initiale Majuskel markiert.
Äußerungsartige Fragmente, die syntaktisch nicht satzfähig sind, können aber ebenfalls wie Sätze interpunktiert werden.
\textit{Nebensätze} und VPs mit Satzgliedstatus (\textit{zu}-Infinitive) werden durch Komma markiert.
}
