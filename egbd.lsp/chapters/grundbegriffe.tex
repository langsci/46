...
Im Folgenden wird der Begriff der grammatischen Einheit verwendet.
Zunächst also die sehr durchsichtige Definition dieses Begriffs.
...
Linguistische Einheiten auf allen Ebenen haben Merkmale (man könnte auch von Eigenschaften sprechen), so wie wir im Grunde allen Dingen in der Welt Merkmale zusprechen können.
...
Allein durch das Vorhandensein oder die Abwesenheit eines Merkmals werden also Dinge klassifiziert, unabhängig von den jeweiligen Werten der Merkmale.

\subsection{Grammatische Merkmale und Werte}
...
Ein Verb (\textit{laufen}, \textit{philosophieren} usw.) hat in keiner seiner Formen ein grammatisches Geschlecht (das sog.\ Genus, also Femininum, Maskulinum oder Neutrum).
...
Sobald wir sagen, ein Wort sei ein Nomen oder ein Verb, wissen wir also, dass bestimmte Merkmale bei diesem Wort vorhanden sein müssen, andere aber nicht.
...
Wir geben nun auch eine Definition der Begriffe Merkmal und Wert.
...
\section{Relationen zwischen linguistischen Einheiten}

\label{sec:kat-funk-rel}
...
Alle diese Begriffe haben mit Relationen (Beziehungen) zwischen linguistischen Einheiten zu tun, allerdings auf verschiedene Art und Weise und auf verschiedenen Ebenen.

\subsection{Das Lexikon und Kategorien}
...
Schon im Zusammenhang mit Merkmalen haben wir von Kategorien gesprochen.
Um den Begriff systematisch anzugehen, soll erst der Begriff des Lexikons definiert werden, wobei wir auf den Merkmalsbegriff zurückgreifen.
...
Die Definition hat den Charakter der Vorläufigkeit vor allem deshalb, weil wir noch nicht definiert haben, was überhaupt ein Wort ist, aber die Definition des Lexikons darauf zurückgreift (vgl.\ Kapitel~\ref{sec:wortklassen}, Definition~\ref{def:wort}, S.~\pageref{def:wort}).
Man kann aber vorerst einen nicht definierten, intuitiven Wortbegriff zugrundelegen.%
\footnote{Wie schon beim Begriff der Grammatik ist hier mit Lexikon also nicht ein Nachschlagewerk in Buchform gemeint.}
...
Die Elemente (die Wörter) in der Menge (dem Lexikon) sind allein dadurch geordnet, dass sie in unterschiedlichem Maße identische Merkmale und Werte für diese Merkmale haben.\\
...
Wenn wir Beispielwörter hinzufügen, wird dies wie in Abbildung~\ref{fig:lexkat1woerter} schnell sichtbar.
...
Substantive und Verben (ggf.\ auch Adjektive) haben alle ein solches Merkmal, haben oder bilden also eine Einzahl (Singular) oder Mehrzahl (Plural): \textit{Mann}, \textit{Männer} bzw.\ \textit{laufe}, \textit{laufen}.
Wörter wie \textit{bald}, \textit{unter} oder \textit{dass} haben dieses Merkmal nicht.
Man könnte also die Kategorisierung revidieren und den Baum in Abbildung~\ref{fig:lexkat2} als Analyse vorschlagen, der natürlich auch noch nicht die endgültige Fassung sein kann (vgl.\ Kapitel~\ref{sec:phrasen} und \ref{sec:saetze}).
Die Unterscheidung in Wörter mit und ohne \textsc{Numerus} entspricht der traditionellen Unterscheidung zwischen formveränderlichen (flektierbaren oder auch beugbaren) und nicht formveränderlichen Wörtern.\index{Wort!flektierbar}
...
Wenn man das Lexikon genauer auf diese Weise untersucht, ergibt sich eine vollständige Hierarchie, die die traditionell als Wortarten, besser aber als Wortklassen bezeichneten Kategorien abbildet.
Allerdings wird die Unterscheidung wesentlich feiner als die traditionellen Wortarten, denn jeder Unterschied in der Merkmalsausstattung erzeugt eine neue Unterkategorisierung.
...
\label{def:kategorie}
Eine Kategorie ist eine Menge linguistischer Einheiten, die alle ein bestimmtes Merkmal haben oder bei denen der Wert eines bestimmten Merkmals gleich ist.
\index{Kategorie}
}

Kategorisierungen anhand von Merkmalsausstattungen werden im nächsten Kapitel (Phonetik) eingeführt, zum Beispiel wenn Vokale und Konsonanten unterschieden werden.
Aber auch syntaktische Einheiten wie die sogenannten Satzglieder (\zB Objekte oder adverbiale Bestimmungen) können so definiert werden, dass sich die wesentlichen Unterschiede im grammatischen Verhalten (und damit die Wortart) aus ihren Merkmalsausstattungen ergeben.
...
Zwischen je zwei Wörtern besteht mindestens die Relation \textit{Ist-in-derselben-Klasse}, oder sie besteht eben nicht.
...
\subsection{Paradigmatische Beziehungen}

\label{sec:parasyn}

\subsubsection{Paradigma und Syntagma}

Der Begriff des Paradigmas hat viel mit unserer Definition der Kategorie zu tun.
...
Man erkennt leicht, dass es sich bei (\ref{ex:gb8991})--(\ref{ex:gb8993}) um den Singular (Einzahl) und den Plural (Mehrzahl) der jeweiligen Wörter handelt, und dass im Falle von \textit{laufen} und \textit{kaufen} die Formen der dritten Person des Singulars im Präsens und Präteritum angegeben wurden.
Die Formen (\textit{die}) \textit{Tochter} und (\textit{die}) \textit{Töchter}, die Formen (\textit{der}) \textit{Saum} und (\textit{die}) \textit{Säume} sowie die Formen (\textit{der}) \textit{Mensch} und (\textit{die}) \textit{Menschen} stehen offensichtlich in einer besonderen Beziehung, und diese Beziehung ist systematisch in den Formen aller Substantive zu beobachten, auch wenn die Art der Formenbildung jeweils stark unterschiedlich ist bzw.\ manchmal sogar gar kein Formenunterschied auftritt.
...
Die damit demonstrierten Beziehungen sind paradigmatisch.
...
Wir sprechen von Einheiten statt von Wörtern, weil nicht nur einzelne Wörter, sondern auch kleinere oder größere Einheiten prinzipiell Paradigmen bilden, auch wenn das morphologische Paradigma (die Formen eines Wortes) den prototypischen Fall eines Paradigmas darstellen.
Ein Beispiel für eine paradigmatische Beziehung zwischen größeren Einheiten ist in (\ref{ex:gb8282}) illustriert.
...
Sie unterscheiden sich allerdings in ihrer Form und auch in weiteren Merkmalen, wie \zB dem Vorhandensein (\ref{ex:gb8282a}) oder Nichtvorhandensein (\ref{ex:gb8282b}) eines Subjektes bzw.\ eines Substantivs im Nominativ.

Das Beispiel (\ref{ex:gb8282}) leitet damit auch über zum Begriff des Syntagmas, der den des Paradigmas ergänzt.
...
In bestimmten Positionen des Syntagmas werden dabei typischerweise spezifische Formen aus dem Paradigma der Einheit, die an der Position steht, gefordert.
\index{Syntagma}
}

Der Begriff des Syntagmas wird im Abschnitt zu den syntaktischen Relationen (Abschnitt~\ref{sec:syntaktischerelationen}) wiederkehren, und er ist der zentrale theoretische Begriff, mittels dessen man die Bildung größerer Strukturen aus kleineren Einheiten verstehen kann.
...
\subsubsection{Paradigmen und Merkmale}

\label{sec:paradigmasyntagmamerkmale}
\index{Merkmal}
...
Neben der möglichen aber nicht notwendigen Veränderung einer Form ist also vor allem die zwingende spezifische Setzung eines Werts für bestimmte Merkmale in den einzelnen Formen eines Paradigmas typisch.
Weil es sogar relativ oft vorkommt, dass Formen in einem Paradigma äußerlich übereinstimmen (s.\ Abschnitt~\ref{sec:synkret}), ist es wesentlich günstiger, sich auf die verschiedenen Werte der Merkmale als auf die Form zu beziehen.
...
In den Kapiteln~\ref{sec:nomina} und \ref{sec:verben} wird jeweils ausführlicher auf Motiviertheit oder Unmotiviertheit grammatischer Merkmale eingegangen.
...
\subsubsection{Formen-Synkretismus}
...
Daher widmet sich Abschnitt~\ref{sec:strukturen} den Grundlagen grammatischer Strukturbildung.
...
Die Einteilung der linguistischen Ebenen (vgl.\ Abschnitt~\ref{sec:grebenen}) deutet darauf hin, dass es sich in der Grammatik als sinnvoll erwiesen hat, sprachliche Strukturen als zusammengesetzt aus jeweils kleineren Strukturen anzusehen.
...
Die einzelnen Einheiten in einer Struktur werden Konstituenten (Bauteile) der gesamten Struktur genannt.
...
Innerhalb einer Struktur haben Konstituenten dadurch auch eine Beziehung zu anderen Konstituenten, sie sind Ko-Konstituenten.
...
Eine wichtige Unterscheidung bei der Konstituenz ist die zwischen unmittelbarer Konstituente und mittelbarer Konstituente.
...
\subsection{Syntaktische Relationen}

\label{sec:syntaktischerelationen}

\subsubsection{Allgemeine Definition}

Was wir in Abschnitt~\ref{sec:strukturen} zur Struktur gesagt haben, ist relativ eindimensional.
...
Diese Relationen werden syntaktische Relationen genannt.
Man kann diese Relationen auch syntagmatische Relationen nennen, weil sie auf allen Ebenen der Strukturbildung (nicht nur in der Syntax) angenommen werden.
...
Es folgen einige Beispiele, um diese Definition mit Leben zu füllen.
...
Zwei wichtige syntaktische Relationen, an denen dies deutlich wird, werden in Abschnitt~\ref{sec:rektion} und Abschnitt~\ref{sec:begriffkongruenz} kurz erklärt.

\subsubsection{Rektion}

\label{sec:rektion}

Dass syntaktische Relationen unbedingt als Teil der Grammatik betrachtet werden müssen, wird dann deutlich, wenn aus ihnen bestimmte Anforderungen an Formen und Merkmale von Einheiten resultieren.
Rektion und Kongruenz verlangen genau dies.
Der letzte Satz von Definition~\ref{def:rektion} ist vielleicht etwas verwirrend, aber wird im Zusammenhang mit der Definition von Kongruenz klarer.

\Definition{Rektion}{
\label{def:rektion}
In einer Rektionsrelation werden durch die regierende Einheit (Regens) Werte für bestimmte Merkmale (und ggf.\ auch die Form) beim regierten Element (Rectum) verlangt.
...
Das Verb hat dabei nichts mitzureden.
...
Wie in Abschnitt~\ref{sec:rektioninderngr} argumentiert wird, ist es sinnvoll, anzunehmen, dass der Genitiv von \textit{meines Kollegen} von \textit{Einsatz} regiert wird.
Dass die Wortfolge \textit{meines Kollegen} im Genitiv steht, liegt aber daran, dass sie von \textit{dank} regiert wird.
...
Damit wird direkt zur Kongruenz übergeleitet.

\subsubsection{Kongruenz}

\label{sec:begriffkongruenz}

Bei der Kongruenz handelt es sich immer um eine nicht-zufällige Übereinstimmung von Merkmalen.
...
\begin{exe}
  \ex{Die Verteidigerinnen lassen keinen Ball durch.}
  \ex{Die Defensive lässt keinen Ball durch.}
\end{exe}
...
\label{sec:valenzundkoepfe}

\index{Valenz}

Mit Valenz bezeichnet man die Eigenschaft von Verben und anderen Wörtern wie Adjektiven (s.\ Abschnitt~\ref{sec:adjektivklassifikation}) und Präpositionen (s.\ Abschnitt~\ref{sec:prpgr}) -- ganz vage gesagt -- das Vorhandensein einer oder mehrerer anderer Konstituenten in einer Struktur zu steuern.
...
Im Kern geht es darum, den unterschiedlichen Status der sogenannten Ergänzungen wie \textit{ein Bild} in (\ref{ex:gb82882}) und der sogenannten Angaben wie \textit{gerne} in (\ref{ex:gb82883}) zu definieren.%
...
Man spricht auch davon, dass Ergänzungen entweder obligatorisch sind, wenn sie stehen müssen, oder fakultativ, wenn sie weglassbar sind.
...
Kritisch sind die fakultativen Fälle wie in (\ref{ex:gb292117}), denn sie sind dafür verantwortlich, dass man nicht (wie früher öfters getan) sagen kann, dass Ergänzungen die Satzteile seien, die bei einem Verb auf jeden Fall stehen müssen.
...
Wir gehen daher hier genau den anderen argumentativen Weg und bauen die Definition darauf auf, dass Ergänzungen solche Einheiten sind, die in manchen Fällen eben nicht stehen dürfen.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex[*]{\label{ex:gb1359c} Ein Atheist glaubt nicht bei einem Schöpfer.}
  \end{xlist}
\end{exe}
...
Man sagt auch, Angaben seien iterierbar (wiederholbar).
...
Auch wenn es später noch zuhilfegenommen wird, soll es hier zunächst nicht verwendet werden.
...
Auch wenn diese Geschichten etwas forciert klingen, wird eine vergleichbare Rettung von Sätzen mit einem Genitiv bei \textit{anschieben} wie in (\ref{ex:gb1359a}) usw.\ nicht gelingen.
...
Unschärfen werden realistisch gesehen auch bei dieser Definition trotzdem gelegentlich übrig bleiben.
...
Valenz ist nun ganz einfach sekundär zum Begriff der Ergänzung zu definieren.
...
Diesen Begriffen entsprechen die eher traditionellen Begriffe von (in derselben Reihenfolge) intransitiv, transitiv und ditransitiv.

\index{Verb!transitiv}
\index{Verb!ditransitiv}
\index{Verb!intransitiv}

Bei Verben wie \textit{glauben} (\textit{an}), die ein Präpositionalobjekt als Ergänzung lizenzieren, spricht man analog auch von präpositional zwei- und dreiwertigen Verben.\label{abs:praepditrans}
