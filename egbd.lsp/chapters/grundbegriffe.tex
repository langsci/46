...
\Zusammenfassung{
Grammatische Einheiten (Laute, Wörter, Sätze usw.) können -- genau wie andere Betrachtungsgegenstände -- über \textit{Merkmale} und ihre \textit{Werte} beschrieben werden.
Die Unterscheidung von Merkmalen (\zB \textsc{Kasus}) und ihren Werten (im Fall von \textsc{Kasus} \textit{Nominativ}, \textit{Akkusativ} usw.) erlaubt es, sprachliche Einheiten allein schon durch das Vorhandensein bzw.\ die Abwesenheit von Merkmalen in Klassen einzuteilen.
}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{(die) Frau =\\{}[\textsc{Laute}: \textit{frau}, \textsc{Genus}: \textit{feminin}, \textsc{Numerus}: \textit{singular}, \textsc{Kasus}: \textit{akkusativ}]}
    \ex{(der) Frau =\\{}[\textsc{Laute}: \textit{frau}, \textsc{Genus}: \textit{feminin}, \textsc{Numerus}: \textit{singular}, \textsc{Kasus}: \textit{dativ}]}
...
\Definition{Struktur}{
\label{def:struktur}
Eine Struktur ist gegeben durch (1) kleinere Einheiten, die als ihre Bestandteile fungieren (\textit{Konstituenten}), und (2) die Reihenfolge, in der diese Bestandteile zusammengesetzt sind.
...
\Zusammenfassung{
Neben der Beschreibung sprachlicher Einheiten geht es in der Grammatik vor allem um \textit{Relationen} zwischen diesen Einheiten.
Diese können kontextunabhängig sein, wie \zB die Relationen zwischen Wörtern im Lexikon.
Darüberhinaus bilden Einheiten aber auch \textit{Kontexte} für andere Einheiten, und nicht jede Art von Einheit kann in jedem Kontext (\textit{Syntagma}) stehen.
...
Bei der \textit{Kongruenz} müssen Merkmalswerte von Einheiten übereinstimmen, bei der \textit{Rektion} verlangen Einheiten bestimmte Werte bei anderen Einheiten.
}
...
Wie schon zu (\ref{ex:gb82882u3}) angedeutet wurde, ist es nun offensichtlich so, dass die Ergänzungen ganz unabhängig von der Bedeutung eines Verbs bei diesem in unterschiedlichem Maß stehen müssen, können oder gar nicht stehen darf.
...
Die Valenz eines Wortes legt fest, welche anderen Einheiten mit diesem Wort im Satz vorkommen dürfen (\textit{Lizenzierung}), und welche mit ihm vorkommen müssen.
Die von einem Valenzgeber lizenzierten Einheiten (\zB Nomina in bestimmten Kasus bei bestimmten Verben) sind seine \textit{Ergänzungen}.
Sie können typischerweise nicht mit allen Arten von Valenzgebern kombiniert werden.
Einheiten, die uneingeschränkt kombinierbar sind, sind \textit{Angaben}.
}
