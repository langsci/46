...
Während schon definiert wurde, was wir unter einem Nebensatz verstehen (Definition~\ref{def:nebensatz} auf S.~\pageref{def:nebensatz} und Abschnitt~\ref{sec:vgr}), wird jetzt definiert, was wir unter einem \textit{unabhängigen Satz} -- landläufig \textit{Hauptsatz} -- verstehen.
...
Sie ordnen syntaktische Strukturen in Kategorien ein, und die Einordnung kann ohne die Betrachtung des Kontextes durchgeführt werden.
...
Der Begriff \textit{Matrix}(\textit{satz}) ist also ein \textit{relationaler} Begriff, der eine Teil-Ganzes-Beziehung beschreibt.

\Definition{Matrix}{
\label{def:matrixsatz}
Die Matrix einer Konstituente ist die Konstituente, in die sie unmittelbar eingebettet ist.
\index{Matrix}
\index{Matrixsatz}
} 

In (\ref{ex:satz08154711}) ist zur Illustration die Matrix jeweils mit [~]$\mathrm{_M}$ und die eingebettete Konstituente mit [~]$\mathrm{_E}$ markiert.
...
Bei Relativsätzen wie in (\ref{ex:satz08154711b}) muss man von der \textit{Matrix-NP} (hier \textit{den Mann}) sprechen, weil der Relativsatz eine unmittelbare Konstituente dieser NP ist, aber nur eine mittelbare Konstituente des ganzen Satzes.
...
Die Rede von der Matrix ist aber im Bereich der Satzsyntax üblicher als in der Phrasensyntax.
...
In Abschnitt~\ref{sec:felder} wird zunächst dafür argumentiert, dass man diese verschiedenen Konstituentenstellungen mittels \textit{Bewegung} von Konstituenten aus der in Abschnitt~\ref{sec:vgr} beschriebenen VP ableiten kann.
Außerdem wird das sogenannte \textit{Feldermodell} eingeführt, dass diese Konstituentenstellungen deskriptiv klassifiziert.
In Abschnitt~\ref{sec:satzschemata} werden dann Phrasenschemata für Sätze angegeben, die alle wichtigen Konstituentenstellungsvarianten beschreiben.
...
Das sogenannte \textit{Feldermodell} liefert eine einfache Terminologie zur Beschreibung der sich durch den Bau der VP und die gerade besprochenen Umsortierungen der Konstituenten im unabhängigen Aussagesatz ergebenden Varianten der Konstituentenstellung.
...
Im eingeleiteten Nebensatz (den wir als KP analysieren) steht der Komplementierer ganz links, und keine Konstituente des Nebensatzes darf links davon stehen.
...
Echofragen sind typisch in Kontexten, in denen der Fragende eine Verständnisfrage stellt, weil er die betreffende Konstituente \zB akustisch nicht verstanden hat.
...
\Zusammenfassung{
Im unabhängigen Aussagesatz steht das finite Verb nicht im Verbalkomplex, sondern an zweiter Stelle nach einer (fast) beliebig wählbaren anderen Konstituente (V2-Satz).
Ein unabhängiger Aussagesatz kann als VP betrachtet werden, aus dem zuerst das finite Verb und dann eine andere Konstituente hinausbewegt wurde.
...
Im Entscheidungsfragesatz wird nur das finite Verb nach links herausgestellt (Verb-Erst-Satz).
}
...
Andere Analysen sind in einem größeren theoretischen Rahmen -- für den hier kein Platz ist -- natürlich möglich und erwünscht.
...
Für solche Relativpronomen können wir annehmen, dass sie im Lexikon bereits durch ein Merkmal als Relativelement gekennzeichnet sind, und dass die Relativphrase, deren Kopf es ist, dieses Merkmal erbt.
...
\begin{figure}[!htbp]
  ...
  \Tree[-0.3]{
    ...
    \POS[]-(0,4)\ar@{-->}@/_{0.5pc}/[rr]-(0,4)_{\textsc{Gen,Num}} && \K{\textit{dessen}} && \K{\textit{Geschmack}}\ar@{-->}@/^{1pc}/[l]-(8,4)^{\textsc{Kas}} && \K{\textit{ich}} && \K{\textit{mag}}\POS[]-(0,4)\ar@{-->}@/^{1pc}/[llll]-(0,4)^{\textsc{Kas}} & \\
  }
...
Manche Sprecher akzeptieren es allerdings auch, wenn der Kasus der Relativphrase obliker ist als der durch Rektion im Matrixsatz geforderte, vgl.\ (\ref{ex:saetze8282821a}).
...
\Zusammenfassung{
In einem Relativsatz bezieht sich das Pronomen als Teil der Relativphrase auf das Bezugsnomen und kongruiert mit ihr in Numerus und Genus.
...
Relativsätze können auch ohne Bezugsnomen als freie Relativsätze auftreten und verhalten sich dann wie eine NP.
Komplementsätze sind Sätze, die eine Valenzstelle (Subjekt oder Objekt) des Matrixverbs füllen (\zB mit \textit{dass}).
Adverbialsätze (\zB mit \textit{während}, \textit{damit} oder \textit{weil}) sind Angaben zum Matrixverb.
Nebensätze können nach rechts aus dem Satz versetzt werden, was bei Komplementsätzen nahezu immer der Fall ist (ggf.\ unter Einsetzung eines Korrelats).
