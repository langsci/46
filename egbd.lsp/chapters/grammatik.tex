...
Es zeigt sich also, dass die Sprachsymbole (Laute, Wörter usw.) ein eigenes \textit{Kombinationssystem} (eine \textit{Grammatik}) haben.
...
Auch wenn in der Linguistik andere Ebenen wie die \textit{Semantik} (Bedeutungslehre), die \textit{Pragmatik} (Lehre vom Sprachgebrauch und sprachlichen Handeln) usw.\ intensiv erforscht werden, ist die Beschreibung der formalen Ebenen ein guter Ausgangspunkt jeder Sprachbetrachtung.
...
Die Gesamtheit der Erbwörter ist der Erbwortschatz.\label{abs:erbwortschatz}
...
Diese Verhältnisse lassen sich mit den Begriffen der \textit{Token-} und \textit{Typhäufigkeit} gemäß Definition~\ref{def:toktyp} beschreiben.%
...
Die Peripherie ist durch kleine Klassen mit niedriger Typhäufigkeit gekennzeichnet.
Besonders, wenn periphere Klassen eine hohe Tokenhäufigkeit aufweisen (wie die Hilfs- und Modalverben), sind sie anfällig die Konservierung historischer Muster, die im Kern nicht mehr vorkommen (mehr in Abschnitt~\ref{sec:kleineverbklassen}).
...
\Zusammenfassung{
Wir konzentrieren uns auf die Aspekte von Sprache, die sich als \textit{Symbolsystem} beschreiben lassen.
Eine Grammatik beschreibt bestimmte \textit{Kombinationen von Symbolen} auf verschiedenen \textit{Ebenen} (Laute, Wörter usw.).
Diese Kombinationen -- und \textit{nur} diese -- sind \textit{grammatisch}.
}
...
Der Grundkonsens wird existierenden Grammatiken entnommen, so dass eine eigene Empirie hier nicht stattfinden muss.
Das hat natürlich in erster Linie damit zu tun, dass dieses Buch eine Einführung in die grammatische Beschreibung und kein eigenständiges wissenschaftliches Werk ist.
...
Das Problem kommt durch die Hintertür, wenn die Grenze zwischen trivialerweise Grammatischem und trivialerweise Ungrammatischem überschritten wird.
...
Ideal wäre in dieser Hinsicht die Version in (\ref{ex:gr9821546}), zumindest wenn wir ihre ästhetische Qualität außer Acht lassen.
...
Nur zur Illustration werden in diesem Buch gelegentlich Beispiele aus dem Deutschen Referenz-Korpus (DeReKo) des Instituts für Deutsche Sprache (IDS) in Mannheim zitiert.
...
Auf Basis der grundsätzlichen Überlegungen in diesem Kapitel werden im weiteren Verlauf des Buches die wichtigen Einzelthemen der Grammatik des gegenwärtigen Deutsch besprochen.
...
\Zusammenfassung{
Es ist nicht die Aufgabe der Sprachwissenschaft, den \textit{richtigen Sprachgebrauch} zu definieren.
Einen richtigen Sprachgebrauch gibt es auch gar nicht, sondern nur historisch gewachsene grammatische \textit{Konventionen} innerhalb von Sprechergemeinschaften.
Die Konventionen erlauben durchaus \textit{Variation}, \zB zwischen verschiedenen Regionen.
Die wichtigste und schwierigste Aufgabe der Grammatik als Wissenschaft ist es, durch \textit{empirische Verfahren} herauszufinden, was diese Konventionen sind und sie zu \textit{beschreiben}.
}
