...
\Definition{Wortklassenfilter}{
  \label{def:wfilter}\index{Filtermethode}
...
Man spricht hier vom \textit{Stärkeparadigma} der Adjektive.\index{Stärke!Adjektiv}
...
\WFiltTree[0cm]{Präposition}{wfilt:prep}{nicht flektierbares Wort}{Hat das Wort eine einstellige Valenz und Kasusrektion?}{Präposition}{anderes}
...
\label{def:nebensatz}
Ein Nebensatz ist eine syntaktische Struktur, die ein finites Verb enthält, das an letzter Stelle steht, innerhalb derer typischerweise alle Ergänzungen und Angaben dieses Verbes enthalten sind.
Nebensätze sind syntaktisch abhängig (können nicht alleine stehen).
\index{Nebensatz}
}
...
Zu den übrig gebliebenen Partikeln und Adverben gehört eigentlich noch die kleine Klasse der \textit{Adkopulas}.
Die weitere Trennung dieser Klassen erfolgt in Abschnitt~\ref{sec:adverbenpartikelnwortklassen} und Abschnitt~\ref{sec:kopulapartikelwortklassen}.
...
Die Abgrenzung der Adverben (und Adkopulas, um die es in Abschnitt~\ref{sec:kopulapartikelwortklassen} geht) von den Partikeln ist eine delikate Angelegenheit.
Syntaktisch gesehen sind Adverben und Adkopulas flexibler im Satz positionierbar als Partikeln. 
Dazu definieren wir zuerst den Begriff \textit{Vorfeldbesetzer} bzw.\ \textit{Vorfeldfähigkeit} in Definition~\ref{def:vorfeldfaehig}.\index{Vorfeld}%
...
Die in (\ref{ex:wa2829}) am Satzanfang stehenden Wörter sind vorfeldfähig oder nicht, und die Grammatikalität bzw.\ Ungrammatikalität der Sätze ergibt sich jeweils aus dieser Eigenschaft.
...
Den Adkopulas fehlt außerdem jegliche Flektierbarkeit, und wir betrachten sie daher als eigene Klasse und nicht als Adjektiv.
