...
Diese wenigen Ausnahmen wie in (\ref{ex:phol777202c}) sind wegen dieses ungewöhnlichen Silbenbaus nicht zum Kern des Systems zu rechnen (Abschnitt~\ref{sec:kernundperipherie}).
...
Da wir /\textipa{\t{pf}}/ wie in \textit{Pfau} und /\textipa{\t{ts}}/ wie in \textit{zieh} sowie das seltene /\textipa{\t{tS}}/ wie in \textit{Chips} als Affrikaten (also ein einziger Konsonant) auffassen (Abschnitt~\ref{sec:affrikatenartikulationsorte}), fallen die Frikative /\textipa{f}/, /\textipa{s}/, /\textipa{S}/, /\textipa{h}/, /\textipa{z}/ und /\textipa{J}/ komplett als zweites Segment im Anfangsrand aus (\ref{ex:phol777203a}).%
\footnote{Außerdem kann die Kombination /\textipa{tJ}/ bzw.\ /\textipa{t\c{c}}/ wie in \textit{tja} oder dem norddeutschen Namen \textit{Tjark} zum System gerechnet werden oder nicht.
...
Ob /\textipa{gn}/ im Anfangsrand also zum Kern des Systems gehört, ist daher eine schwierige Frage.}
...
Zusätzlich fassen wir den Approximant und /\textipa{K}/ wie am Ende von Abschnitt~\ref{sec:anfangsrand} vorgeschlagen zur Gruppe der Liquide zusammen.%
...
Bei der deskriptiven Sichtung in Abschnitt~\ref{sec:endrand} schien der Endrand drei oder mehr Segmente enthalten zu können.
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:phol0800009003c} /\textipa{\t{ts}vEks}/ $\Rightarrow$ \textipa{[\t{ts}vEk+s]} (\textit{zwecks})}
  	\ex{\label{ex:phol0800009003d} /\textipa{\u{a}pt}/ $\Rightarrow$ \textipa{[Pap+t]} (\textit{Abt})}
  	\ex{\label{ex:phol0800009003d} /\textipa{n\u{a}kt}/ $\Rightarrow$ \textipa{[nak+t]} (\textit{nackt})}
  	\ex{\label{ex:phol0800009003d} /\textipa{l\t{aO}fs}/ $\Rightarrow$ \textipa{[l\t{aO}f+s]} (\textit{Laufs})}
  \end{xlist}
\end{exe}
...
Extrasilbisch treten /\textipa{s}/ und /t/ hinter den Endrand.}
...
Dieses Muster der Silbenfolge ist charakteristisch für das Deutsche (s.\ Abschnitt~\ref{sec:wortakzentimdeutschen}).
...
Extrasilbische Segmente tragen niemals zur Morenzahl bei.}
...
\Satz{Silbenlänge}{\label{satz:silbenlaenge}
Die Silben des Deutschen sind ein- bis dreimorig.
Einmorige Silben können keinen Einsilbler bilden:
Das phonologische Wort ist mindestens zweimorig.
Daraus folgt, dass Einsilbler immer entweder zwei- oder dreimorig sind.}
...
Das ist auch so, denn während \textit{Ahn} /\textipa{a:n}/ ein einwandfreier Einsilbler ist, ist */\textipa{a:ng}/ (phonetisch *\textipa{[a:N]}) dies nicht.

Auf Basis einer parallelen Argumentationen \textit{müssen} alle extrasilbischen /t/ und /s/ aus Abschnitt~\ref{sec:systematikderraender} extrasilbisch sein, wenn die Bedingung aus Satz~\ref{satz:silbenlaenge} gelten soll.
...
Damit können wir auch die Endränder in \textit{Mensch} und \textit{Ramsch} oder \textit{Milch} und \textit{falsch} aber gut begründet davor bewahren, auch noch zu einem simplexen Endrand nebst einem extrasilbischen /\textipa{S}/ zerstückelt zu werden.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:phol200469a} \textit{freches}: \textipa{[fKE.\c{c}@es]}, *\textipa{[fKE\c{c}.@s]}}
...
Die inkorrekten Silbifizierungen in (\ref{ex:phol200469}) enthalten keine Silben, die in sich schlecht sind.
...
Im Kernwortschatz werden zweisilbige Stämme auf der ersten Silbe akzentuiert.
\index{Akzent!Stamm--}
}
...
Mit dem Betonungstest aus Abschnitt~\ref{sec:prosodie} kann für beliebig lange Komposita festgestellt werden, dass der Akzent immer auf ihrem ersten Bestandteil liegt, vgl.\ (\ref{ex:fokuskomp}).
