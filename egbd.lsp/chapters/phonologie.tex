...
Auch in anderen Sprachen, die den obligatorisch konsonantischen Wortanlaut nicht haben, ist dieser Fall eher selten.
...
Wörter wie wie \textit{qualmt}, \textit{qualmst} oder \textit{Herbsts} zeigen, dass es drei-, vier- und fünfsegmentale Endränder zu geben scheint.
...
Wenn einfaches \textipa{[N]} wie in \textit{Gang} als zugrundeliegende Kombination /ng/ analysiert wird, wird die Verteilung der Endränder gemäß Abbildung~\ref{fig:endranduplex} klar.
...
Wenn der Endrand des Einsilblers duplex ist wie in \textit{wink}, sind gespannte Vokale ebenfalls nicht möglich, wie das unmögliche Wort *\textipa{[vi:Nk]} zeigt.
...
Das ist auch so, denn während /\textipa{an}/ (\textit{an}) und\ \textipa{[a:n]} (\textit{Ahn}) einwandfreie Einsilbler sind, ist /\textipa{ang}/ bzw.\ *\textipa{[a:N]} dies nicht.
...
Sonst wäre ein Einsilbler wie \textit{ahnt} mit \textipa{[a:nt]} bereits viermorig und damit zu schwer, Wörter wie \textit{ahnst} erst recht.
...
In diesen Silben -- bzw.\ \textit{allen}, die durch Abbildung~\ref{fig:endranduplex} beschrieben werden -- ist prinzipiell ein gespannter Vokal ausgeschlossen, s.\ (\ref{ex:phol2003}).
...
Phonetisch ähnlich sind sich \textipa{[@]} und \textipa{[K]} aber nur, wenn man relativ komplexe Merkmalsinventare aus der theoretischen Phonologie zugrundelegt.
...
\begin{enumerate}\Lf
	\item Wird komplett neu gemacht.
\end{enumerate}
...
\Uebung \label{u46} Beschreiben Sie die Phonologie der Wörter \textit{Chaos} und \textit{Chaot} möglichst vollumfänglich.
