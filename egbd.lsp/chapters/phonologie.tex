...
Dann folgt eine recht ausführliche Analyse des Silbenbaus (Abschnitt~\ref{sec:silbenundwoerter}).
Abschließend gibt Abschnitt~\ref{sec:wortakzent} einen Einblick in die Prosodie (die Betonungslehre) und die damit in phonologische Aspekte auf der Wortebene.
...
Es stellt sich die damit entscheidende Frage, ob zwei Segmente die gleiche Verteilung oder eine teilweise oder vollständig unterschiedliche Verteilung haben.
...
Vgl.\ Abschnitt~\ref{sec:woerterwortformen}.
...
Zugrundeliegende phonologische Formen schreibt man konventionellerweise nicht in \textipa{[~]} sondern in /~/, also \zB /\textipa{veg}/, /\textipa{bad}/ und /\textipa{lab}/.%
...
Nach /\textipa{i}/, /\textipa{y}/, /\textipa{I}/, /\textipa{Y}/, /\textipa{e}/, /\textipa{\o}/, /\textipa{E}/, /\textipa{\u{E}}/, /\textipa{\oe}/ kommt \textipa{[\c{c}]} vor, nach /\textipa{u}/, /\textipa{U}/, /\textipa{o}/, /\textipa{O}/, /a/ und /\textipa{\u{a}}/ hingegen \textipa{[X]}.
...
Aufbauend auf der Beschreibung der einzelnen Segmente kann und sollte beschrieben werden, wie diese Segmente zu größeren Einheiten zusammengesetzt werden, wie also die \textit{phonologische Struktur} aufgebaut wird (zum Strukturbegriff vgl.\ Abschnitt~\ref{sec:strukturbildung}).
...
Der Nasal /\textipa{N}/ sowie der Frikative /\textipa{\c{c}}/ kommen prinzipiell im Anfangsrand nicht vor und werden aus allen weiteren Überlegungen über diese Position ausgeschlossen.%
...
Plosive an zweiter Position sind im zweisegmentalen Anfangsrand nahezu unmöglich -- vgl.\ (\ref{ex:phol777202a}) -- mit der Ausnahme von /\textipa{p}/ und /\textipa{t}/ nach /\textipa{S}/ wie in (\ref{ex:phol777202a}).
...
Etwas häufiger sind die Kombinationen /\textipa{nf}/ und /\textipa{ns}/.
...
Daher und aus weiteren Gründen, die in Abschnitt~\ref{sec:systematikderraender} und Abschnitt~\ref{sec:einsilblerzweisilbler} erläutert werden, wird dafür plädiert, diese Segmente als \textit{extrasilbisch} (außerhalb der normalen Silbenstruktur stehend) zu analysieren (Definition~\ref{def:extrasilbisch}).

\Definition{Extrasilbizität}{\label{def:extrasilbisch}
Die Silbenstruktur kann durch vor dem Anfangsrand oder nach dem Endrand stehende \textit{extrasilbische} Segmente ergänzt werden, die nicht der Sonoritätskontur unterliegen.}
...
Um diese Phänomene geht es in Abschnitt~\ref{sec:systematikderraender} nochmals.
...
Da das extrasilbische Material eine Silbe benötigt, um überhaupt auftreten zu können, ist es legitim, eine erweiterte Silbe anzunehmen, die die eigentliche Silbe und das extrasilbische Material zusammenfasst.

\begin{figure}
  \centering
  \Tree[3]{
    && \K{erweiterte Silbe}\B{dd}\B{ddll}\B{ddrr} \\ 
	\\
    \K{extrasilbisches}\Below{Material}& & \K{Silbe}\B{dl}\B{d} && \K{extrasilbisches}\Below{Material} \\
    &\K{Anfangsrand} & \K{Reim}\B{d}\B{dr} \\
    && \K{Kern} & \K{Endrand}\\
  }
...
In Abschnitt~\ref{sec:systematikderraender} werden Plateaus allerdings eliminiert, indem Plateaus bildendes Material als extrasilbisch aufgefasst wird.
...
Wenn man /\textipa{S}/ diesen Sonderstatus zuweist, dampft die Beschreibung des simplexen Anfangsrands auf Abbildung~\ref{fig:anfangsrandsimplex} und die des duplexen Anfangsrands auf Abbildung~\ref{fig:anfangsrandduplex} ein.
...
Die präferierte Besetzung des duplexen Anfangsrands ist die aus einem inneren Liquid und einem äußeren Obstruent.
...
\begin{figure}
  ...
  \Tree[3]{
     ...
     & \K{k g}\R{r}^{\text{Plosiv}} & \K{n}\R{ddrr}_{\text{Nasal}} &&  \\
     \K{\textipa{S}}\R[--]{r} & \K{p t}\R{dr}^{\text{Plosiv}} &&&  \\
     & \K{b d k g}\Rxxx{6}{10.25}{8.25}{ur} & \K{\textipa{K}}\Rxxx{3.7}{3}{-2.25}{ddr} && \K{~~~Vokal} \\
     & \K{f v}\R{ur}_{\text{Frikativ}} &&&  \\
     \K{\textipa{S}}\R[--]{r} & \K{p}\R{dr}^{\text{Plosiv}} &&&  \\
     & \K{b k g}\Rxxx{6}{10.25}{8.25}{ur} & \K{l}\R{uuurr}_{\text{Liquid}} &&  \\
...
\begin{figure}
  ...
  \K{\textbf{Kern}} && \K{\textbf{Endrand}}\Below{\textbf{erste Pos.}} & \K{\textbf{Endrand}}\Below{\textbf{zweite Pos.}} \\
               &&             & \K{k (g)} \\
               && \K{n}\Rxx{3.7}{5}{r}_{\text{Frikativ}}\Rxx{3.7}{5}{ur}^{\text{Plosiv}} & \K{\textipa{f S}} \\
  ...
  \K{Vokal~~~}\R{uurr}^{\text{Nasal}}\R{ddrr}_{\text{Liquid}} &&& \K{\textipa{S}} \\
               &&             & \K{p k} \\
               && \K{\textipa{K l}}\Rxx{3.7}{5.5}{r}^{\text{~~~~~~~Frikativ}}\Rxx{3.7}{4.5}{ur}^{\text{Plosiv}}\R{dr}_{\text{Nasal}} & \K{\textipa{f S \c{c}}} \\
               &&             & \K{m n} \\
  }
...
Die Wörter in (\ref{ex:phol7528a}) und enthalten ein /s/, dass die Sonoritätskontur verletzt.
...
Einerseits fehlt /\textipa{N}/ vollständig, andererseits werden Sequenzen /ng/ beschrieben, obwohl /g/ als stimmhafter Obstruent im Endrand eigentlich gar nicht vorkommen sollte.%
...
Für diese Reduktion des Systems wird in Abschnitt~\ref{sec:einsilblerzweisilbler} weiter argumentiert.
Es fällt aber auf, dass häufig -- wenn auch nicht immer -- extrasilbisches Material (konkret /t/, /s/ oder /st/) zu sogenannten \textit{Flexionsendungen} gehört, also nicht zum sogenannten \textit{Wortstamm} (vgl.\ Abschnitt~\ref{sec:stamm}).
...
Vor der weiteren Vertiefung der strukturellen Zusammenhänge in Abschnitt~\ref{sec:einsilblerzweisilbler} halten wir fest, dass die Besetzungspräferenzen (Satz~\ref{satz:endrandbesetzung}) im Endrand nahezu spiegelbildlich dieselben wie Anfangsrand sind.%
\footnote{Als echte Auslassung im Interesse einer eleganteren Darstellung wurde in Abbildung~\ref{fig:endranduplex} die Besetzung des Endrands mit /\textipa{Kl}/ wie in \textit{Kerl} nicht dargestellt.
Diese ist im Anfangsrand nicht möglich.}

\begin{exe}
  \ex \label{ex:phol0800009005}
  \begin{xlist}
  	\ex{\label{ex:phol0800009005a} /\textipa{kOKb}/ $\Rightarrow$ \textipa{[k\t{O@}b]} (\textit{Korb})}
  	\ex{\label{ex:phol0800009005b} /\textipa{vIKbst}/ $\Rightarrow$ \textipa{[v\t{I@}b+st]} (\textit{wirbst})}
...
Die präferierte Besetzung des duplexen Endrands ist die aus einem inneren Liquid und einem äußeren Obstruent.
Sekundär wird mit einem Nasal und einem homorganen Plosiv besetzt. 
Extrasilbisch treten die alveolaren Obstruenten /\textipa{s}/ und /t/ hinter den Endrand.}
...
Nach den Silben ist die nächstgrößere Einheit der phonologischen Strukturbildung das phonologische Wort.
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:phol0815i} liebte \textipa{[li:b.t@]}}
...
Einsilbler mit ungespanntem (kurzen) Vokal müssen geschlossen sein, s.\ (\ref{ex:phol0815b}) und (\ref{ex:phol0815d}).
Wenn der Vokal gespannt (lang) ist, kann die Silbe offen sein wie in (\ref{ex:phol0815a}), muss es aber nicht, vgl.\ (\ref{ex:phol0815c}).
...
Die Bedingung, dass Silben mit ungespanntem (kurzen) Vokal einen gefüllten Endrand haben müssen, gilt im Zweisilbler nicht, wie (\ref{ex:phol0815h}) zeigt.
...
\Definition{More}{\label{def:more}
Die Länge einer Silbe ist die Anzahl der Moren im Reim der Silbe.
Kurze Vokale im Kern und Konsonanten im Endrand zählen als eine More.
Lange Vokale und Diphthonge zählen als zwei Moren.
Extrasilbische Segmente tragen nicht zur Morenzahl bei.}

Die More zählt nur die Segmente im Reim, der dadurch als Analyseeinheit zusätzlich motiviert wird.
...
Um die Verteilung der Vokallängen in offenen und geschlossenen Silben sowohl in Einsilblern als auch in Mehrsilblern zu erklären und zu vereinheitlichen, lassen wir zu, dass in Mehrsilblern ein Segment gleichzeitig im Endrand einer Silbe und im Anfangsrand der Folgesilbe steht.
Wir schaffen also die kurzen offenen -- und damit einmorigen -- Silben für das Deutsche ganz ab und führen mit Definition Definition~\ref{def:silbengelenk} das \textit{Silbengelenk} in die Beschreibung ein.
...
Eventuelle phonetische Evidenz für diese Analyse kann hier aus Platzgründen nicht besprochen werden, aber der systematische Beschreibungsvorteil einer Analyse mit Silbengelenk lässt sich trotzdem demonstrieren.
Oben haben wir festgestellt, dass kurze offene Silben nicht als Einsilbler vorkommen können.
...
In Fällen wie \textipa{[mIt@]} steht das \textipa{[t]} sowohl im Anfangsrand der zweiten Silbe und im Endrand der ersten Silbe.
Es gibt dafür keine einheitliche Notation, wir schreiben \textipa{[mI\Sub{t}.t@]}.
Abbildung~\ref{fig:silbgel001} zeigt die Analyse mit Silbengelenk.
Es kann nicht überbetont werden, dass am Silbengelenk phonetisch nicht zwei Konsonanten vorliegen (also eben nicht *\textipa{[mIt.t@]}, wie die überzogene Aussprache der Klatschmethode eventuell suggeriert, s.\ Abschnitt~\ref{sec:silben}), sondern ein einziger Konsonant, der in zwei Positionen einer Struktur steht.%
\footnote{Aus diesem Grund lehnen manche Phonologen die Theorie des Silbengelenks ab.
Zu dieser Kritik gehört auch der Einwand, dass es kein phonetisches Korrelat des Silbengelenks gibt, zumindest keins auf das man sich bisher einigen konnte.}
...
In Satz~\ref{satz:silbenlaengemitgelenk} können damit weitreichende Generalisierungen über Längen von deutschen Silben formuliert werden.
Für Einsilbler ergibt sich dank Satz~\ref{satz:silbenlaengemitgelenk} eine minimale Zweimorigkeit direkt aus der minimalen Zweimorigkeit der Silben.

\Satz{Silbenlänge mit Silbengelenk}{\label{satz:silbenlaengemitgelenk}
Unter der Annahme des Silbengelenks sind alle Silben (außer unbetonbaren Silben, also Schwa-Silben) entweder zweimorig (langer Vokal in offenen Silben oder kurzer Vokal mit simplexem Endrand) oder dreimorig (langer Vokal mit simplexem Endrand oder kurzer Vokal mit duplexem Endrand).
In scheinbar kurzen offenen Erstsilben von Mehrsilblern wird Zweimorigkeit dadurch hergestellt, dass der Konsonant im Anfangsrand der Folgesilbe durch seinen Status als Silbengelenk zum Silbengewicht der Erstsilbe gerechnet wird.}

Diese Generalisierung stützt das radikal reduktionistische Vorgehen bei der Beschreibung des Endrands in Abschnitt~\ref{sec:systematikderraender} erheblich.
Zunächst wäre die Entscheidung zu motivieren, /ng/ statt */\textipa{N}/ anzunehmen.
Nach der vorgeschlagenen Analyse besteht der Reim in Wörtern wie \textit{lang} aus drei Segmenten, nämlich /ang/ (statt /\textipa{aN}/).
Dann wäre es zu erwarten, dass an der Position des /a/ keine langen Vokale oder Diphthonge stehen können.
Das ist auch so, denn während /\textipa{an}/ bzw.\ \textipa{[a:n]} ein einwandfreier Einsilbler ist, ist /\textipa{ang}/ bzw.\ *\textipa{[a:N]} dies nicht.
...
Sonst wäre ein Einsilbler wie \textit{ahnt} mit \textipa{[a:nt]} bereits viermorig und damit zu lang, Wörter wie \textit{ahnst} erst recht.
...
In diesen Silben -- bzw.\ \textit{allen}, die durch Abbildung~\ref{fig:endranduplex} beschrieben werden -- ist prinzipiell ein langer Vokal ausgeschlossen, s.\ (\ref{ex:phol2003}).
Wir können jetzt angeben, \textit{warum} (im Sinne einer Systembeschreibung) diese Längenverhältnisse sind, wie sie sind, und nach welcher Systematik in Silben und Wörtern die Segmente einander folgen.
...
Das zunächst vielleicht unauffällige Wort \textit{Bagger} ist relativ frisch aus dem Niederländischen entlehnt.
...
Durch Klatschen hätten sich alle diese Erkenntnisse und diese elegante Beschreibung sicher nicht rekonstruieren lassen.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\label{ex:phol200469a} \textit{freches} \textipa{[fKE.\c{c}@s]}, *\textipa{[fKE\c{c}.@s]}}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Sie hat am \Akz LANGSTRECKEN\Nakz LAUF (und nichts anderem) teilgenommen.}
    \ex{Sie hat sich an dem \Akz BUS\Nakz HALTE\Nakz STELLEN\Nakz UNTERSTAND (und nichts anderem) verletzt.}
  \end{xlist}
\end{exe}
...
Eine andere Lösung erlaubt \textit{defekte Füße}.
Bei \textit{wa\Akz rum} würde es sich demnach um eine Folge von zwei defekten Füßen oder um einen defekten Trochäus \textit{\Akz rum} mit einer vorausgehenden \textit{extrametrischen} Silbe handeln.
Die extrametrische Silbe ist eine Silbe außerhalb eines Fußes, also im Grunde das Äquivalent zu einem extrasilbischen Segment auf der nächsthöheren Ebene. 
...
\textit{Anfangsrand} wird mit A, \textit{Endrand} mit E, \textit{Kern} mit K und \textit{Reim} mir R abgekürzt.
...
\begin{figure}
	\centering
	\Tree{
	\K{} & \K{} & \K{} & \K{} & \K{PhW}\B{d}\B{ddlll} & \K{} & \K{} & \K{} \\ 
	\K{} & \K{} & \K{} & \K{} & \K{F}\B{d}\B{drr} & \K{} & \K{} & \K{} \\ 
...
Statt eines zugrundeliegenden Konsonanten kann der Glottalverschluss eingesetzt werden.}
...
Wir schließen mit einer maximalen Analyse des recht langen Wortes \textit{Rattenfängerinnenverein} in Abbildung~\ref{fig:rattenfaengerinnenverein}.
...
\begin{figure}
 ...
 \resizebox{\textwidth}{!}{
 \Treek[-1]{1.5}{
 \K{} & \K{PrW}\B{d}\B{drrrrrr}\B{drrrrrrrrrrrrrrrrrrr} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} \\ 
 \K{} & \K{PhW}\B{d} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{PhW}\B{d}\B{drrrrr} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{PhW}\B{ddll}\B{d} & \K{} \\ 
 \K{} & \K{F}\B{d}\B{drrr} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{F}\B{d}\B{drrr} & \K{} & \K{} & \K{} & \K{} & \K{F}\B{d}\B{drrr} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{} & \K{F}\B{d} & \K{} \\ 
 \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{} & \K{S}\B{ddl}\B{d} & \K{} & \K{S}\B{ddl}\B{d} & \K{} \\ 
 \K{} & \K{R}\B{d}\B{dr} & \K{} & \K{} & \K{R}\B{d}\B{dr} & \K{} & \K{} & \K{R}\B{d}\B{dr} & \K{} & \K{} & \K{R}\B{d} & \K{} & \K{R}\B{d}\B{dr} & \K{} & \K{} & \K{R}\B{d}\B{dr} & \K{} & \K{} & \K{R}\B{d} & \K{} & \K{R}\B{d}\B{dr} & \K{} \\
 \K{A}\B{d} & \K{K}\B{d} & \K{E}\B{dr} & \K{A}\B{d} & \K{K}\B{d} & \K{E}\B{d} & \K{A}\B{d} & \K{K}\B{d} & \K{E}\B{dr} & \K{A}\B{d} & \K{K}\B{d} & \K{A}\B{d} & \K{K}\B{d} & \K{E}\B{dr} & \K{A}\B{d} & \K{K}\B{d} & \K{E}\B{d} & \K{A}\B{d} & \K{K}\B{d} & \K{A}\B{d} & \K{K}\B{d} & \K{E}\B{d} \\
 \K{\textipa{K}} & \K{\textipa{a}} & \K{\textipa{}} & \K{\textipa{t}} & \K{\textipa{@}} & \K{\textipa{n}} & \K{\textipa{f}} & \K{\textipa{E}} & \K{\textipa{}} & \K{\textipa{N}} & \K{\textipa{@}} & \K{\textipa{K}} & \K{\textipa{I}} & \K{\textipa{}} & \K{\textipa{n}} & \K{\textipa{@}} & \K{\textipa{n}} & \K{\textipa{f}} & \K{\textipa{5}} & \K{\textipa{P}} & \K{\textipa{\t{ae}}} & \K{\textipa{n}} \\
 }
 }
 \label{fig:rattenfaengerinnenverein}
 \caption{Phonologische Analyse des Wortes \textit{Rattenfängerinnenverein}}
\end{figure}
...
In der hier benutzten Darstellung lässt sich die Auslautverhärtung kompakt als Prozess der Änderung eines Merkmals unter einer bestimmten Bedingung formulieren (vgl.\ Abschnitt~\ref{sec:auslautverhaertungphonologie}).
...
Man würde \textit{Lab} dann als /l\textipa{a:}B/ repräsentieren und damit anzeigen, dass anstelle des Archiphonems /B/ positionsbedingt verschiedene Allophone zu erwarten sind.
Es kommt dem Konzept zugrundeliegenden Form konzeptuell recht nahe.
Der Nachteil ist allerdings, dass /b/ in \textit{Rübe} und /B/ in /l\textipa{a:}B/ zwei verschiedene zugrundeliegende Segmente sind.
...
\footnote{Ein anonymer Gutachter der ersten Auflage wies mich darauf hin, dass phonembasierte Theorien (auch wissenschaftsgeschichtlich gesehen) durchaus mit Merkmalen formuliert werden und wurden.
...
Vor allem gegen ein solches Vorgehen wird hier also argumentiert.}
Die Phone \textipa{[h]} und \textipa{[N]} sind im Deutschen zueinander nicht distinktiv (vgl.\ Abschnitt~\ref{sec:segmentemerkmaleverteilungen}, vor allem (\ref{ex:phol6439}) auf S.~\pageref{ex:phol6439}).
...
Bei \textipa{[\c{c}]} und \textipa{[X]} ist die komplementäre Verteilung hingegen eindeutig nicht zufällig, wie in Abschnitt~\ref{sec:verteilungvonichach} demonstriert wurde.
...
Ebenso wäre im Anfangsrand \textipa{[K]} ein Allophon von /\textipa{r}/ (vgl.\ Abschnitt~\ref{sec:rvokalisierungen}).%
\footnote{Hier wird absichtlich /\textipa{r}/ als Symbol für das Phonem verwendet, um deutlich zu machen, dass es sich eben nicht um eine zugrundeliegende Form handelt und man daher irgendein Symbol hernehmen kann.
...
\Uebung \label{u46} Beschreiben Sie die Phonologie der Wörter \textit{\Akz Chaos} und \textit{Cha\Akz ot} möglichst vollumfänglich.

\Uebung[\tristar] \label{u47} Warum kann \textipa{[s5]} im Deutschen kein Einsilbler sein?
