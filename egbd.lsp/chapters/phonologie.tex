...
In \textit{Herbsts} liegt nach dem /b/ sogar eine Kontur aus Anstieg, Abstieg und erneutem Anstieg vor, s.\ Abbildung~\ref{fig:sonhierherbsts}.
...
\begin{figure}
  ...
  \begin{tabular}{cccccccc}
    ...
    P & & & & \rnode{herbsts04}{\textipa{b}} & & \rnode{herbsts06}{\textipa{t}} & \\
  \end{tabular}
...
Ein /t/ nach /m/ und \textipa{[N]} wie in \textit{klemmt} oder \textit{hängt} kann dann als extrasilbisch analysiert werden.
Wenn einfaches \textipa{[N]} wie in \textit{Gang} dann zusätzlich als zugrundeliegende Kombination /ng/ analysiert wird, wird die Verteilung der Endränder gemäß Abbildung~\ref{fig:endranduplex} klar.
...
Für diese Reduktion des Systems wird in Abschnitt~\ref{sec:einsilblerzweisilbler} weiter argumentiert, da /ng/ sich auch in anderer Hinsicht wie zwei Segmente verhält.
...
Diese ist im Anfangsrand weder in dieser Reihenfolge noch spiegelbildlich.}
...
Einsilbler mit ungespanntem kurzen Vokal müssen geschlossen sein, vgl.\ \textit{Schiff} und das unmögliche Wort *\textipa{[knI]}.
Wenn der Vokal gespannt und lang ist, kann die Silbe offen sein wie in \textit{Knie}, muss sie aber nicht, vgl.\ \textit{schief}.
Wenn der Endrand des Einsilblers duplex ist wie in \textit{wink}, sind gespannte lange Vokale ebenfalls nicht möglich, wie das unmögliche Wort *\textipa{[vi:Nk]} zeigt.
Die Bedingung, dass Silben mit ungespanntem kurzen Vokal einen gefüllten Endrand haben müssen, gilt im Zweisilbler nicht, wie \textit{Mi.tte} demonstriert.
Ansonsten gilt aber trotzdem, dass Silben nicht einen gespannten langen Vokal im Kern und gleichzeitig einen komplex besetzten Endrand haben können, s.\ *\textipa{[v\t{i5}k.t@]} verglichen mit \textit{wirkte}.
...
Eine Morenlänge haben nur die Segmente im Reim, der dadurch als Analyseeinheit zusätzlich motiviert wird.
...
Dort steht V für kurze Vokale, \=V für lange Vokale und Diphthonge und C für einen Konsonanten.
Die Tabelle kann folgendermaßen gelesen werden:
Einmorig sind nur offene Schwa-Silben.
Zweimorig sind offene Silben mit langem Vokal und Silben mit kurzem Vokal und simplexem Endrand.
Dreimorig sind Silben mit langem Vokal und simplexem Endrand sowie Silben mit kurzem Vokal und duplexem Endrand.
...
In scheinbar kurzen offenen Erstsilben von Mehrsilblern wird Zweimorigkeit dadurch hergestellt, dass der Konsonant im Anfangsrand der Folgesilbe durch seinen Status als Silbengelenk zum Silbengewicht der Erstsilbe zählt.}

\begin{table}
	\centering
	\begin{tabular}{lccc}
		\lsptoprule
		 & \textbf{Kern} & \multicolumn{2}{c}{\textbf{Endrand}} \\
		\midrule
		\textbf{einmorig} & \textipa{@} & & \\
		\midrule
		\multirow{2}{*}{\textbf{zweimorig}} & \=V &  &  \\
		& V & C & \\
		\midrule
		\multirow{2}{*}{\textbf{dreimorig}} & \=V & C & \\
		& V & C & C \\
		\lspbottomrule
	\end{tabular}
