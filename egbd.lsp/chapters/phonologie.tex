...
\begin{figure}
  \centering
  \begin{tabular}{l|p{6pt}cccccp{6pt}|r}
    \cline{2-8}
    && & & & & && \\
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:phol0800009003c} /\textipa{ʧvEks}/ $\Rightarrow$ \textipa{[ʧvEk+s]} (\textit{zwecks})}
...
Ansonsten gilt aber trotzdem, dass Silben nicht einen gespannten (langen) Vokal im Kern enthalten und gleichzeitig einen komplex besetzten Endrand haben können, s.\ (\ref{ex:phol0815k}).
...
\begin{enumerate}\Lf
  \item Überlegen Sie, wie sich im Fall von Lehnwörtern wie \textit{Chemie} oder \textit{Chuzpe} die teilweise üblichen Realisierungen wie \textipa{[\c{c}emi:]} und \textipa{[XUʧp@]} in das phonologische System des Deutschen integrieren.
  ...
  \item Bedenken Sie die Tatsache, dass für \textit{Chuzpe} niemals \textipa{[SUʧp@]} oder \textipa{[kUʧp@]} realisiert werden.
