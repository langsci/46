...
Abschließend gibt Abschnitt~\ref{sec:wortakzent} einen Einblick in die \textit{Prosodie} (die \textit{Betonungslehre}) und damit in phonologische Aspekte auf der Wortebene.
...
\index{Verteilung!komplement{\"a}r}
}
...
Es ergibt sich das neue Vokaltrapez in Abbildung~\ref{fig:vokalviereckmitgespannt}, das um den Preis erkauft wird, dass \textipa{[E]} und \textipa{[a]} jeweils bald als gespannte, bald als ungespannte Varianten angesehen werden.
...
Wenn man alle Formen eines Worts berücksichtigt, kann man Änderungen von Merkmalswerten beobachten.
...
Diese Regularitäten beschreibt gemäß Definition~\ref{def:phonotaktik} die Phonotaktik.
...
Innerhalb der Silbe gibt es das universelle Bildungsprinzip der \textit{Sonoritätskontur}, welches regelt, dass die Sonorität zum Vokal hin ansteigt und dann wieder abfällt, wie in Abbildung~\ref{fig:sonhier} schematisch dargestellt.\index{Sonorit{\"a}t}
...
\Definition{Extrasilbizität}{\label{def:extrasilbisch}\index{Extrasilbizit{\"a}t}%
...
Es fällt außerdem auf, dass häufig -- wenn auch nicht immer -- extrasilbisches Material (konkret [t], [s] oder [st]) zu sogenannten \textit{Flexionsendungen} gehört, also nicht zum sogenannten \textit{Wortstamm} (vgl.\ Abschnitt~\ref{sec:stamm}).\index{Extrasilbizit{\"a}t!und Flexionssuffixe}
...
Ein ungespannter Vokal im Kern und ein einzelner Konsonant im Endrand zählen jeweils als eine \textit{More}.
...
\index{Silbe!Gelenk}
\index{Silbe!Ambisyllabizit{\"a}t}
}
...
\index{Silbe!Gelenk!Auslautverh{\"a}rtung}\label{abs:silbengelenkstimmlos}
...
\index{Akzent!Pr{\"a}fixe und Partikeln}
}
...
\Definition{Fuß}{\label{def:fuss}\index{Fu{\ss}}%
...
Das \textit{phonologische Wort} ist die aus Füßen (in vereinfachter Darstellung aus Silben) bestehende Einheit, innerhalb derer die Regularitäten der segmentalen Phonologie und der Phonotaktik wirken.
Das \textit{prosodische Wort} ist die aus phonologischen Wörtern bestehende Einheit, innerhalb derer prosodische Regularitäten (Akzentzuweisung) wirken.
\index{Wort!phonologisch}
