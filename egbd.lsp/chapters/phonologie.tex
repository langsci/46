...
Zur Unterscheidung in der Notation schreiben wir hier /\textipa{a;}/ /\textipa{E;}/ für die \textit{ungespannten} Versionen.
...
Ob /\textipa{gn}/ im Anfangsrand also zum Kern gehört, kann bezweifelt werden.}
...
Von diesen Verbindungen sind viele (\zB /\textipa{mp}/) sehr selten.
...
Als Kombinationen aus Nasal und Frikativ kommt /\textipa{n\c{c}}/ kommt wohl nur in zwei nennenswert häufigen Wörtern vor, s. (\ref{ex:phol4715a}).
...
In Wörtern wie \textit{trittst} setzt sich das Problem fort, weil nach dem Anstieg ein weiterer Abfall folgt.
...
Um diese geht es in Abschnitt~\ref{sec:anfangsrandendrand} nochmals.
...
Scheinbare Fälle von drei Segmenten (/\textipa{SpK}/, /\textipa{StK}/ und evtl.\ /\textipa{[Spl}/) im Anfangsrand bestehen aus zwei Segmenten mit extrasilbischem /\textipa{S}/.
...
Alle Kombinationen, die eine Verletzung der Sonoritätskontur darstellen würden, werden dabei zunächst nicht beachtet.
Außerdem wird /\textipa{N}/ aus dem System eliminiert (mehr dazu weiter unten).
Es ergibt sich Abbildung~\ref{fig:endranduplex}, die den duplexen Endrand ohne extrasilbisches Material abbildet.
Mit wenigen Zusatzannahmen ergibt sich daraus der gesamte komplexe Endrand.

\begin{figure}
  \Tree[3]{
  \K{\textbf{Kern}} && \K{\textbf{Endrand}}\Below{\textbf{erste Pos.}} & \K{\textbf{Endrand}}\Below{\textbf{zweite Pos.}} \\
               && \K{\textipa{f S s \c{c}}}\Rxx{6}{3.7}{r}^{\text{Plosiv}} & \K{t} \\
               &&             & \K{t k g} \\
               && \K{n}\Rxx{3.7}{5}{r}_{\text{Frikativ}}\Rxx{3.7}{5}{ur}^{\text{Plosiv}} & \K{\textipa{f s S}} \\
               & \K{}\Rxxxx{5}{1.75}{3.7}{1.2}{r} & \K{m}\R{r}^{\text{Plosiv}}\R{dr}_{\text{Frikativ}} & \K{p t} \\
  \K{Vokal~~~}\Rxx{3.7}{6}{uuuurr}^{\text{Frikativ}}\R{uurr}_{\text{Nasal}}\R{ddrr}_{\text{Liquid}} &&& \K{\textipa{S s}} \\
               &&             & \K{p t k} \\
               && \K{\textipa{K l}}\Rxx{3.7}{5.5}{r}^{\text{~~~~~~~Frikativ}}\Rxx{3.7}{4.5}{ur}^{\text{Plosiv}}\R{dr}_{\text{Nasal}} & \K{\textipa{f s S \c{c}}} \\
               &&             & \K{m n} \\
  }
...
Das Diagramm in Abbildung~\ref{fig:endranduplex} beschreibt nicht alle scheinbar duplexen Endränder.
Wörter wie in (\ref{ex:phol7528}) müssen anders erklärt werden, wenn Abbildung~\ref{fig:endranduplex} allgemein gelten soll.

\begin{exe}
  \ex \label{ex:phol7528}
  \begin{xlist}
  	\ex{\label{ex:phol7528a} Huts}
  	\ex{\label{ex:phol7528b} Schnaps, Huts, zwecks}
  	\ex{\label{ex:phol7528c} Abt, nackt}
  	\ex{\label{ex:phol7528d} Laufs, Reichs}
  \end{xlist}
\end{exe}

Die Wörter in (\ref{ex:phol7528a}) und (\ref{ex:phol7528b}) enthalten ein /s/, dass die Sonoritätskontur verletzt.
Wie schon im Anfangsrand behandeln wir es als extrasilbisch.
Das /t/ in (\ref{ex:phol7528c}) bildet mit den vorangehenden Plosiven ein Sonoritätsplateau.
Wie sich zeigen wird, muss /t/ in einigen Fällen extrasilbisch sein, wenn ein vorangehendes /s/ ebenfalls bereits extrasilbisch ist.
Zudem sind sowohl /t/ als auch /s/ alveolare Obstruenten, und wir nehmen daher an, dass diese beiden extrasilbisch an den Endrand treten können.
Damit verschwinden Endränder mit mehr als zwei Segmenten vollständig aus dem System, wie noch demonstriert werden wird.
Das /s/ in (\ref{ex:phol7528d}) kann ebenfalls extrasilbisch interpretiert werden, so dass auch keine Frikativ-Plateaus angenommen werden müssen.
...
\begin{table}
  \begin{tabular}{ccccccccp{1cm}l}
                   &             &             &                               &   &     & t & s && \textit{Huts}   \\                   &             &             &                               &   & p k &   & s && \textit{Schnaps}, \textit{zwecks} \\
                   &             &             &                               &   & p k & t &  && \textit{Abt}, \textit{nackt}    \\
                   &             &             & f \textipa{S \c{c}}           & s &   & t &   && \textit{Luft}, \textit{reicht}, \textit{acht}, \textit{Ast}  \\
     ...
     \textipa{K} l &             &             & \textipa{S}                   &   &   &   &   && \textit{falsch}, \textit{Kirsch}     \\
...
Der laryngale Plosiv \textipa{[P]} ist nicht zugrundeliegend und wird im Zuge der Akzentzuweisung und der Silbifizierung in den leeren Anfangsrand von Silben eingefügt, die entweder (1) am Wortanfang stehen oder (2) im Wortinneren stehen und betont sind.
}

Silben, die eigentlich einen leeren Anfangsrand haben (also mit Vokal anlauten) werden um dieses Segment unter genau benennbaren phontaktischen und prosodischen Bedingungen ergänzt.
