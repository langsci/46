...
Da wir /\textipa{\t{pf}}/ wie in \textit{Pfau} und /\textipa{\t{ts}}/ wie in \textit{zieh} sowie das seltene /\textipa{\t{tS}}/ wie in \textit{Chips} als Affrikaten (also ein einziger Konsonant) auffassen (Abschnitt~\ref{sec:affrikatenhomorgan}), fallen die Frikative /\textipa{f}/, /\textipa{s}/, /\textipa{S}/, /\textipa{h}/, /\textipa{z}/ und /\textipa{J}/ komplett als zweites Segment im Anfangsrand aus (\ref{ex:phol777203a}).%
...
Im komplexen Anfangsrand sind häufig (im Sinn einer Typenhäufigkeit, s.\ Abschnitt~\ref{sec:kern}) vor allem Kombinationen aus Plosiv und /\textipa{K}/ oder /\textipa{l}/.
...
Zusätzlich fassen wir den Approximant und /\textipa{K}/ wie am Ende von Abschnitt~\ref{sec:anfangsrandeskriptiv} vorgeschlagen zur Gruppe der Liquide zusammen.%
\footnote{Dabei ist zusätzlich zu bedenken, dass /\textipa{K}/ im Endrand phonetisch als Vokal artiuliert wird.}
...
Die nicht explizit aus anderen Gründen ausgeschlossenen Segmente treten alle in simplexen Endrändern des Kernwortschatzes auf.
...
Anders als im Anfangsrand sind zunächst Folgen aus zwei Plosiven möglich, allerdings nur \textipa{[pt]} und \textipa{[kt]} -- von allen sechs theoretischen Möglichkeiten.
...
Es kann nur ein Plosiv folgen, im Fall von /f/, /s/ und /\textipa{\c{c}}/ ist dies aber immer /\textipa{t}/ wie in (\ref{ex:phol4713}).
Völlig ausgeschlossen ist /\textipa{S}/.

\begin{exe}
  \ex{\label{ex:phol4713} Luft, Lust, Licht}
\end{exe}

Außerdem können alle Frikative (bis auf /\textipa{s}/) mit einem folgendem \textipa{[s]} kombiniert werden, vgl.\ (\ref{ex:phol47135}).

\begin{exe}
  \ex{\label{ex:phol47135} Laufs, Reichs, Bachs}
\end{exe}
...
Von den Frikativen kommen /f/, /s/ und /\textipa{\c{c}}/ in Frage wie in (\ref{ex:phol4716b}), /\textipa{S}/ nicht.
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:phol4716b} elf, Hals, Milch, darf, Kurs, Lurch}
...
Ein schrittweises induktives Vorgehen würde den Rahmen sprengen, und das Gesamtsystem wird daher in Abschnitt~\ref{sec:anfangsrandendrand} kompakt aufgerollt.
...
\begin{figure}
  ...
  \begin{tabular}{l|ccccc|r}
    \cline{2-6}
    (minimal sonor) & \rnode{HP}{P} & \rnode{HF}{F} & \rnode{HN}{N} & \rnode{HA}{L} & \rnode{HV}{V} & (maximal sonor) \\
    \cline{2-6}
  \end{tabular}
...
Vor Frikativen (\textit{Schwung}, \textit{Schrank}) entsteht ein Sonoritätsplateau.
Lediglich in mehrsegmentalen Anfangsrändern mit Nasal oder Approximant an zweiter Stelle (\textit{Schmal}, \textit{Schlund}) verhält sich /\textipa{S}/ konform zur Sonoritätshierarchie.
...
Die maximale Komplexität des Anfangsrands besteht also in zwei Segmenten (duplex).
...
Bei der deskriptiven Sichtung in Abschnitt~\ref{sec:endranddeskriptiv} schien der Endrand drei oder mehr Segmente enthalten zu können.
...
\begin{figure}
  ...
  \K{Vokal~~~}\R{uurr}_{\text{Nasal}}\R{ddrr}_{\text{Liquid}} &&& \K{\textipa{S}} \\
               &&             & \K{p k} \\
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:phol08000090035c} /\textipa{lEst}/ $\Rightarrow$ \textipa{[lEs+t]} (\textit{lässt})}
  \end{xlist}
\end{exe}
...
Folgt man der Verzweigung Vokal -- Nasal -- /n/ -- Plosiv, dann findet man oben rechts ein /g/.
Warum es eingeklammert ist, wird im weiteren Verlauf erklärt.}
Mögliche zweisegmentale Kombinationen mit \textipa{[N]} an der phonetischen Oberfläche zeigen Wörter wie \textit{hängt}, \textit{krank} und \textit{längs}.
...
Phonologisch haben wir es in /gang/ mit einem duplexen Endrand zu tun, phonetisch mit einem simplexen.
...
Die Beziehung von zugrundeliegender Form und phonetischer Oberfläche in den kritischen Formen illustriert (\ref{ex:phol0800009004}).
...
Auch hier werden /s/ und /t/ wo möglich als extrasilbisch gewertet.
...
\Satz{Endrandbesetzung}{\label{satz:endrandbesetzung}
...
\begin{exe}
  ...
  \begin{xlist}
  	...
  	\ex{\label{ex:phol0815j} wirkte \textipa{[vIKk.t@]}}
  	\ex{\label{ex:phol0815k} \textipa{[vi:Kk.t@]}}
  \end{xlist}
\end{exe}

Die Wörter in (\ref{ex:phol0815}) sind entweder einsilbig, oder sie bestehen aus einer Silbe mit einem betonten gespannten (und langen) oder einem betonten ungespannten (und kurzen) Vokal, die von einer Schwa-Silbe gefolgt wird.
...
Ansonsten gilt aber trotzdem, dass Silben nicht einen gespannten (langen) Vokal und einen komplexen Endrand haben können.
...
Mit einer verwandten Argumentationen müssen alle extrasilbischen /t/ und /s/ aus Abschnitt~\ref{sec:anfangsrandendrand} extrasilbisch sein, wenn die Bedingung aus Satz~\ref{satz:silbenlaenge} gelten soll.
...
Aufgrund dieser Argumentation können wir auch die Endränder in \textit{Mensch} und \textit{Ramsch} oder \textit{Milch} und \textit{falsch} davor bewahren, in einen simplexen Endrand und einem extrasilbischen /\textipa{S}/ zerhackt zu werden.
...
Selbst wenn alle Silben in einem Wort den reichhaltigen bisher besprochenen Strukturbedingungen genügen müssen, bleiben zahlreiche Zweifelsfälle, wo genau denn die Grenze zwischen Silben in Mehrsilblern zu ziehen ist.
...
\Satz{Maximierung des Anfangsrands}{Die Silbifizierung von Mehrsilblern erfolgt so, dass an Silbengrenzen die Anzahl der Segmente im Anfangsrand der zweiten Silbe so groß wie möglich ist.
...
Durch die Maximierung des Anfangsrands (Abschnitt~\ref{sec:maxanfangsrand}) gerät dabei das /\textipa{K}/ von \textit{Leserin} in den Anfangsrand der letzten Silbe, und es wird folgerichtig nicht vokalisiert, so wie es bei \textit{Leser} passiert.
...
In diesem optionalen Abschnitt soll kurz auf einige oft erwähnte phonologische Begriffe -- vor allem auf den des Phonems -- eingegangen werden.
...
Ebenso wäre im Anfangsrand \textipa{[K]} ein Allophon von /\textipa{r}/ (vgl.\ Abschnitt~\ref{sec:prozrvok}).%
