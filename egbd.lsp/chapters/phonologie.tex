...
Zwei Wörter, die sich nur in einem Segment an derselben Position unterscheiden, nennt man \textit{Minimalpaar}, und ein Minimalpaar illustriert jeweils einen \textit{phonologischen Kontrast}.\index{Minimalpaar}
...
\Satz{Auslautverhärtung}{Segmente mit [\textsc{Obstruent}:~$+$] sind am Silbenende [\textsc{Stimme}:~$-$].}
...
Trotzdem ist das gespannte \textipa{[a]} phonetisch nicht vom ungespannten \textipa{[a]} unterscheidbar, und Gleiches gilt für gespanntes und ungespanntes \textipa{[E]}.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{\label{ex:phol777272e} \textit{Ökonomi} \textipa{[P\o konomi:]}\\
      \textit{maneuvrieren} \textipa{[man\o vKi:K@n]} }
...
Es wird in dieser Position als Schwa-Segment (\textipa{[@]} oder \textipa{[5]}) realisiert.}
...
Wenn man alle Formen eines Worts berücksichtigt (\zB \textit{Bad} und \textit{Bades}), kann man Änderungen von Merkmalswerten beobachten (\textipa{[ba:t]} vs.\ \textipa{[ba:d@s]}).
...
Komplizierter wird die Silbenphonologie dadurch, dass in den verschiedenen Formen eines Wortes die Silbengrenzen nicht immer konstant sind.
...
Aus Gründen, die erst in Abschnitt~\ref{sec:einsilblerzweisilbler} diskutiert werden, hat es sich als nützlich erwiesen, Kern und Endrand zu einer eigenen Konstituente, dem \textit{Reim} zusammenzufassen.\index{Reim}
...
\Definition{Liquid}{\label{def:liquid}Liquide sind \textit{l}- und \textit{r}-Segmente.
...
Alle zulässigen Silbenstrukturen stellen einen Anstieg der Sonorität zur Mitte der Silbe und einen Abfall der Sonorität zum Ende der Silbe (oder einen Ausschnitt aus so einem Verlauf) dar.
\index{Sonorität}
}
...
Die maximale Komplexität des Anfangsrands besteht also in zwei Segmenten:\index{Anfangsrand!komplex}
...
Extrasilbisch tritt ggf.\ \textipa{[S]} vor den Anfangsrand.\index{Anfangsrand!komplex}}
...
Bei der deskriptiven Sichtung in Abschnitt~\ref{sec:endrandimeinsilbler} schien der Endrand drei oder mehr Segmente enthalten zu können.\index{Endrand!komplex}
...
\Satz{Endrand}{\label{satz:endrandbesetzung}\index{Endrand!komplex}
...
Zunächst müssen Einsilbler mit ungespanntem Vokal geschlossen sein, \zB \textit{Schiff} im Vergleich mit unmöglichen Wörtern wie *\textipa{[knI]} oder auch *\textipa{[tO]} usw.
Das gilt im übrigen auch für die letzte Silbe in Mehrsilblern, so dass *\textipa{[kUn.dI]} oder *\textipa{[tu:.bO]} ausgeschlossen sind.
...
Wenn der Endrand des Einsilblers duplex ist wie in \textit{wink}, sind gespannte Vokale allerdings nicht möglich, wie das strukturell unmögliche Wort *\textipa{[vi:Nk]} zeigt.
...
\Zusammenfassung{
Wörter bestehen phonotaktisch betrachtet aus einer oder mehreren \textit{Silben}, die jede mindestens einen \textit{vokalischen Kern} haben.
...
Der Akzent ist eine Prominenzmarkierung, die einer Silbe im phonologischen Wort zugewiesen wird.
...
Ein Fuß besteht aus einer oder mehreren Silben, und jedes phonologische Wort besteht aus einem oder mehreren Füßen.
...
\Definition{Defekte Füße und extrametrische Silben}{\label{def:defektefuesseextrametrischesilben}\index{Silbe!extrametrisch}\index{Fuß!defekt}%
...
In dieser Vertiefung soll kurz auf einige oft verwendete phonologische Begriffe -- vor allem auf den des \textit{Phonems} -- eingegangen werden.%
