...
\Definition{Phonologischer Kontrast (Segment)}{
\label{def:phokonseg}
Zwei phonetisch unterschiedliche Segmente stehen in einem phonologischen Kontrast, wenn diese Segmente eine teilweise oder vollständig übereinstimmende Verteilung haben und dadurch einen lexikalischen bzw.\ grammatischen Unterschied markieren können.
\index{Kontrast}
}
...
Zugrundeliegende phonologische Formen schreibt man dann nicht in \textipa{[~]} sondern in /~/, also \zB /\textipa{\t{ts}vEKg}/, /\textipa{ba:d}/ und /\textipa{la:b}/.%
...
In Definition~\ref{def:phokonseg} (S.~\pageref{def:phokonseg}) wurde gesagt, dass mit unterschiedlichen Segmenten phonologische Kontraste markiert werden können, \zB /\textipa{d}/ vs. /\textipa{t}/ in /\textipa{daNk}/ und /\textipa{taNk}/.
...
\Definition{Phonologischer Kontrast (Merkmal)}{
\label{def:phokonfeat}
Ein phonologisches Merkmal erzeugt einen phonologischen Kontrast, wenn es durch einen Unterschied seiner Werte in mindestens einer Umgebung einen lexikalischen bzw.\ grammatischen Unterschied markieren kann.
\index{Kontrast}
}
...
\Satz{Phonologische Merkmale}{
Für die Beschreibung einer Sprache werden genau soviele Merkmale benötigt, dass jeder phonologische Kontrast abgebildet werden kann.
\index{Merkmal!phonologisch}
}
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\textsc{Kons(onant)}: $+$, $-$}
    \ex{\textsc{Approx(imant)}: $+$, $-$}
    \ex{\textsc{Son(orant)}: $+$, $-$}
    \ex{\textsc{Kont(inuant)}: $+$, $-$}
  \end{xlist}
\end{exe}
...
\begin{figure}[!h]
  ...
  \Tree[3]{
    && \K{Segment}\B{dl}\B{dr} \\
    & \K{[\textsc{Kons}:$+$]}\B{dl}\B{dr} && \K{[\textsc{Kons}:$-$]}\Below{/\textipa{i: I y: Y u: U e: o: O \o: \oe\ E: E a: a}/} \\
    \K{[\textsc{Approx}:$+$]}\Below{/\textipa{l}/} && \K{[\textsc{Approx}:$-$]}\B{dl}\B{dr} \\
    & \K{[\textsc{Son}:$+$]}\Below{/\textipa{m n N}/} && \K{[\textsc{Son}:$-$]}\B{dl}\B{dr} \\
    && \K{[\textsc{Kont}:$+$]}\Below{/\textipa{f v s z S \c{c} J K h}/ \textipa{[X]}} && \K{[\textsc{Kont}:$-$]}\Below{/\textipa{p b t d k g}/} \\
  }
...
\Definition{Kontinuant}{
Ein Kontinuant ist ein Segment, bei dem der pulmonale Luftstrom weitgehend ungehindert durch den Mundraum abfließt.
\index{Kontinuant}
}
...
\begin{exe}
  ...
  \ex
  \begin{xlist}
    \ex{\textsc{Ort}: \textit{labial, koronal, dorsal, laryngal}}
    \ex{\textsc{Hinten}: $+$, $-$}
  \end{xlist}
\end{exe}
...
\Definition{Koronal}{
\label{def:koronal}
Koronale sind Segmente, die mit der Zungenspitze artikuliert werden (phonetisch: dental, alveolar, palato-alveolar).
\index{Koronal}
}

\Definition{Dorsal}{
\label{def:dorsal}
Dorsale sind Segmente, die mit dem Zungenrücken artikuliert werden (phonetisch: palatal, velar, uvular).
\index{Dorsal}
}

\Definition{Laryngal}{
\label{def:guttural}
Laryngale sind Segmente, die im Kehlkopfbereich artikuliert werden (phonetisch ebenfalls laryngal).
\index{Laryngal}
}

Bei den Koronalen und Dorsalen wird nach hinten und nicht hinten unterschieden.

\Definition{Hinten (Koronale/Dorsale)}{
\label{def:konshinten}
Hintere Koronale sind solche, die hinter den Alveolen artikuliert werden.
Hintere Dorsale sind solche, die hinter dem Velum artikuliert werden.
\index{hinten}
}

Nicht hinten sind bei den Koronalen also die alveolaren, und hinten die palato-alveolaren.
Die Unterscheidung ist im Deutschen notwendig, um die ansonsten identischen /\textipa{s}/ und /\textipa{S}/ zu trennen.%
\footnote{Wenn die Segmente in Lehnwörtern wie \textit{Genre} und \textit{Chips} ebenfalls beschrieben werden sollen, trennt dieses Merkmal auch /\textipa{z}/ und /\textipa{Z}/ sowie /\textipa{\t{ts}}/ und /\textipa{\t{tS}}/.
}
Im Bereich der Dorsalen sind die Uvularen \textipa{[X]} und /\textipa{K}/ hinten.
Nicht-hintere Dorsale sind \zB /\textipa{k}/ und /\textipa{g}/.
...
\begin{figure}[!h]
  \resizebox{\textwidth}{!}{
    \Tree[2.5]{
      &&& \K{Segment}\B{dlll}\B{dl}\B{drr}\B{drrrr} \\
      \K{[\textsc{Ort}: \textit{labial}]}\Below{/\textipa{p b m f v}/} && \K{[\textsc{Ort}: \textit{koronal}]}\B{dl}\B{dr} &&& \K{[\textsc{Ort}: \textit{dorsal}]}\B{dl}\B{dr} && \K{[\textsc{Ort}: \textit{laryngal}]}\Below{/\textipa{h}/} \\
      & \K{[\textsc{Hinten}:$-$]}\Below{/\textipa{t d n s z l}/} && \K{[\textsc{Hinten}:$+$]}\Below{/\textipa{S}/} & \K{[\textsc{Hinten}:$-$]}\Below{/\textipa{k g N \c{c} J}/} && \K{[\textsc{Hinten}:$+$]}\Below{/\textipa{K}/ \textipa{[X]}} & \\
    }
  }
...
\textsc{Gespannt} ist ein neues Merkmal, das in der Phonetik nicht eingeführt wurde.
...
Phonologisch hilft es uns aber auf jeden Fall, /\textipa{E}/ und /\textipa{e:}/ sowie /\textipa{o:}/ und /\textipa{O}/ usw.\ zu unterscheiden.
...
Weiterhin lässt sich relativ gut zusammenfassen, welche Folgen von gleich sonoren Lauten in Onset un Coda auftreten können.%
...
\subsection{Segmentale Prozesse}

\label{sec:segmentaleprozesse}
