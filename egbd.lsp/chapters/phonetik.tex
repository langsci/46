...
Die auditive Phonetik beschreibt, wie Sprachlaute von Gehör und Gehirn verarbeitet werden.
Die akustische Phonetik beschreibt Sprachlaute hinsichtlich ihrer physikalischen Qualität als Schallwellen.
\index{Phonetik}
}

Eine wichtige Aufgabe der artikulatorischen Phonetik ist dabei, ein Notationssystem zu entwickeln, mit dem Sprachlaute möglichst eindeutig und sehr genau notiert werden können.
...
\BVertiefung{Phonetik und Sprachsystem}{Im Grunde ignorieren wir mit unserer Auffassung von Phonetik unsere Definition von Sprache als System von Symbolen und Regularitäten (vgl.\ \zB \ref{def:grammatik}).
Wir fragen hier nämlich, wie ein Mensch die Symbole konkret hervorbringt, bewegen uns also eigentlich im Bereich einer spezifischen praktischen Umsetzung der Symbole.
Nach unserer Definition würde es reichen, zu sagen, dass es bestimmte voneinander unterscheidbare Laute gibt, eine Analyse nach Art und Ort ihrer Artikulation wäre überflüssig.
Die Grammatik so anzufangen, wäre allerdings übermäßig abstrakt.
Wir machen hier sozusagen ein sehr sinnvolles Zugeständnis an die Tatsache, dass wir irgendein konkretes Medium für unsere Symbole brauchen, und dieses Medium oft Sprachlaute sind.
In Teil~\ref{part:schrift} wird das andere wichtige Medium besprochen, nämlich die Schrift.
}
...
Die Schreibung wird in diesem Fall benutzt, um einen Wortklassen-Unterschied (nicht etwa einen lautlichen Unterschied) zu markieren, denn die Buchstaben(folgen) \textit{s} und \textit{ss} entsprechen hier absolut identischen Lauten.\index{Buchstabe}
...
\Tu{Um diese Vorgänge nachzuvollziehen, können Sie sich direkt nach dem Ausatmen Nase und Mund zuhalten und versuchen, einzuatmen.
...
\Tu{Halten Sie die Luft an und versuchen dann, zu sprechen.
...
\Tu{Wenn Sie sich beim Sprechen vor einen Spiegel stellen oder an den Kehlkopf fassen, sehen bzw.\ merken Sie, wie er sich leicht auf und ab bewegt.}
...
\Tu{Wenn Sie sich an den Kehlkopf/die Kehlkopfgegend fassen und verschiedene Wörter langsam sprechen (\zB \textit{Achat}, \textit{Verwaltungsangestellter}), werden Sie merken, dass der Kehlkopf bei einigen Lauten (\textit{a}, \textit{w}, \textit{ng} usw.) eine Vibration produziert, bei anderen (\textit{ch}, \textit{t} usw.) aber nicht.}
...
\Tu{Sprechen Sie Wörter mit vielen \textit{h}-Lauten am Silbenanfang aus, \zB \textit{Haha}, \textit{Hundehalter} usw.
...
\Tu{Ihren Rachen können Sie sehen, wenn Sie sich vor einen Spiegel stellen, die Zunge mit einem geeigneten Gegenstand herunterdrücken und \textit{ah} sagen.
...
\Tu{Von Ihrer Zunge sehen Sie, wenn Sie sich vor den Spiegel stellen, nur den kleinsten Teil, nämlich den beweglichen Rücken und die bewegliche Spitze.
...
\Tu{Man kann das Gaumensegel ertasten, indem man mit der Zunge oder einem Finger vorsichtig im Gaumen nach hinten fährt.
...
\Tu{Halten Sie sich die Nase zu und sprechen Sie zunächst langanhaltend \textit{f} und \textit{s}, dann \textit{m} und \textit{n}.
Mit zugehaltener Nase sollte es nicht möglich sein, die \textit{m}- und \textit{n}-Laute kontinuierlich auszusprechen.
Das liegt daran, dass bei diesen die Luft durch die Nasenhöhle statt durch die Mundhöhle abfließt.
Insofern ist die Nasenhöhle indirekt an der Produktion dieser Laute beteiligt.
}
...
\Tu{Sprechen Sie langsam und sorgfältig das Wort \textit{Tante} und achten Sie darauf, wo sich die beweglichen Teile Ihres Mundraums jeweils befinden.
Sowohl die beiden \textit{t}-Laute als auch die beiden \textit{n}-Laute sind durch eine Berührung der Zunge an einer bestimmten Stelle innerhalb des Mundraums charakterisiert.
...
Was die Artikulatoren bei welchen Lauten genau machen, wird \textit{Artiulationsart} genannt und in den folgenden Abschnitten klassifiziert und illustriert.

\Definition{Artikulationsart}{\label{def:artart} Die Artikulationsart eines Lautes ist die Art und Weise, in der der Luftstrom aus der Lunge durch die Artikulatoren behindert wird.}
...
Ein Sprachlaut ist stimmhaft, wenn nahezu zeitgleich zu seiner Artikulation ein Stimmton produziert wird.
\index{Stimmhaftigkeit}
}

\Tu{Um sich den Unterschied nochmals vor Augen zu führen, sprechen Sie folgende Wortpaare (möglichst überdeutlich) aus und fassen sich dabei an die Kehlkopfgegend, um das Vibrieren des Kehlkopfes (oder dessen Fehlen) zu fühlen: \textit{sehen} (\textit{s} stimmhaft), \textit{krass} (\textit{ss} stimmlos), \textit{Wanne} (\textit{w} stimmhaft), \textit{fahren} (\textit{f} stimmlos).}

\subsection{Obstruenten}

Bei der zuerst zu besprechenden Gruppe von Sprachlauten handelt es sich um die sogenannten Obstruenten.
Nach der Definition folgen Abschnitte über die Unterarten von Obstruenten.

\Definition{Obstruent}{
Ein Obstruent ist ein Sprachlaut, bei dem der pulmonale Luftstrom durch eine Verengung, die die Artikulatoren herstellen, am freien Abfließen gehindert wird.
...
\Tu{Halten Sie sich eine Handfläche dicht vor den Mund und sprechen Sie folgende Wörter sorgfältig aus: \textit{Kuckuck}, \textit{Torte}, \textit{Pappe}.
...
\Definition{Plosiv}{
Ein Plosiv ist ein Obstruent, bei dem einer totalen Verschlussphase eine Lösung des Verschlusses folgt und ein Knall- oder Knacklaut entsteht.
\index{Plosiv}
}
...
%\footnote{Es wird gerne gesagt, die stimmlosen Plosive im deutschen Standard seien aspiriert.
%Aspiration liegt dann vor, wenn, nach der Lösung des Plosivs ein nicht eigenständiger laryngaler Reibelaut (vulgo: Hauch) artikuliert wird, was in IPA dann \zB \textipa{[t\super h]} notiert wird.
%Dies ist leicht hörbar in keinem deutschen Dialekt der Fall.
%Zwar ist der Druck in der Verschlussphase in standardnahen Dialekten hoch (nicht aber \zB im Fränkischen und anderen südlichen Dialekten), und die Verschlusslösung geht daher meist mit einer Koartikulation einher, aber es handelt sich nicht um Aspiration, sondern im Normalfall um eine leichte Reibung an der Artikulationsstelle, also \textipa{[t\super θa:k\super x]} für \textit{Tag} usw.
%Diese Phänomene sind aber eigentlich komplexer (vgl.\ \textipa{[k\super JInt\super θ]} für \textit{Kind}, \textipa{[vIs@nSaft\super ɬlI\c{c}]} für \textit{wissenschaftlich} usw.) und von geringerer Wichtigkeit, weswegen sie hier der didaktischen Reduktion zum Opfer fallen.}

\subsubsection{Frikative}

\Tu{Sprechen und fühlen Sie folgende Wörter: \textit{Skischuhe}, \textit{Fach}, \textit{Wicht}.
Bei den Lauten, die durch \textit{sch} (und in \textit{Ski} ausnahmsweise \textit{sk}), \textit{f}, \textit{ch} und \textit{w} wiedergegeben werden, spüren Sie ein konstantes, mehr oder weniger scharfes Entweichen von Luft.}
...
Der beteiligte Plosiv und der beteiligte Frikativ sind dabei homorgan (an derselben Stelle gebildet).
\index{Affrikate}
}
...
\Definition{Vibrant (Trill)}{
Ein Vibrant ist ein Sprachlaut, bei dem der aktive Artikulator im Luftstrom zum schnellen wiederholten Schlagen gegen den passiven Artikulator angeregt wird.
\index{Vibrant}
}
...
\Tu{Beobachten Sie, wie im Wort \textit{Ball} der letzte Laut gebildet wird.}
...
Ein lateraler Approximant ist ein Sprachlaut, bei dem neben einem zentralen Verschluss der Artikulatoren der Luftstrom weitgehend ungehindert ohne Bildung von Turbulenzen abfließt.
\index{Approximant}
}
...
\Definition{Nasal}{
Ein Nasal ist ein Sprachlaut, bei dem durch einen vollständigen Verschluss im Mundraum (und eine Absenkung des Velums) die Luft zum Entweichen durch die Nasenhöhle gezwungen wird.
...
\Tu{Sprechen Sie sich die Vokallaute vor und beobachten Sie dabei (einschließlich Beobachtung im Spiegel), wie sich die Zunge, die Lippen und die sonstigen Organe im Mundraum dabei verhalten.}
...
\Definition{Vokal}{
\label{def:vokal}
Ein Vokal ist ein Sprachlaut, bei dem der pulmonale Luftstrom weitgehend ungehindert abfließen kann, und bei dem keine geräuschhaften Anteile entstehen.
Der Klang eines Vokals wird durch eine spezifische Formung des Resonanzraumes charakterisiert.
\index{Vokal}
}

\Tu{Wenn Sie bei der Produktion von Vokalen wieder Ihren Kehlkopf ertasten, werden Sie feststellen, dass alle stimmhaft sind.}
...
Sonoranten (Klanglaute) sind nicht-geräuschhafte Sprachlaute, bei denen der pulmonale Luftstrom ohne Bildung von Turbulenzen durch den Mund oder die Nase abfließen kann.
Alle anderen Sprachlaute gelten als geräuschhaft und werden Obstruenten (Geräuschlaute) genannt.
\index{Sonorant}
\index{Obstruent}
}
...
Es sind die Laute, die typischerweise (aber nicht notwendigerweise) nicht silbisch sind, also prototypischerweise alleine keine Silbe bilden können.
\index{Konsonant}
}
...
\Definition{Artikulationsort}{\label{def:artort} Der Artikulationsort eines Lautes ist der Punkt der größten Annäherung zwischen den Artikulatoren.}
...
\Tu{Wenn Sie Wörter wie \textit{Anpfiff} oder \textit{energisch} sehr deutlich und energisch aussprechen, hören Sie am Anfang des Wortes einen Plosiv, einen Knacklaut im Kehlkopf.
...
%Ein einfaches Experiment, das dies für jeden Sprecher zweifelsfrei zeigt, ist es, wenn dieser Sprecher ein Wort wie \textit{Buch} spricht und den letzten Laut möglichst lange artikuliert.
%Einen schwachen Brechreiz vorausgesetzt kann dieser Sprecher dann seinen vollständigen weichen Gaumen ertasten, was nicht möglich wäre, wenn dort der Zungenrücken zur Artikulation anläge.
}
...
\Tu{Zur bewussten Lokalisierung von \textipa{[X]} und \textipa{[K]}, die im hinteren Bereich der Mundhöhle gebildet werden, hilft es, die vordere Zunge mit einem geeigneten Gegenstand herunterzudrücken und dann \zB \textit{Rache} zu sagen (mit \textipa{[K]} und \textipa{[X]}).
...
\Tu{Halten Sie wieder die Zungenspitze fest und artikulieren Sie \textit{King Kong} und \textit{Gang}.
Die Artikulation sollte ähnlich gut gelingen wie bei \textit{Rache}, weil auch hier die Zungenspitze nicht beteiligt ist.
Mit ein bisschen Mühe ist es möglich, den Ort und die Art der Artikulation dieser Laute im Selbstversuch auch visuell zu beobachten.
...
\Tu{Sprechen Sie die folgenden Wörter und achten Sie auf die Anlaute: \textit{lang}, \textit{schön}, \textit{Tor}, \textit{Didi}.
Diese Laute werden am unteren Teil des Zahndamms gebildet.
...
\Tu{Wenn Sie wieder ein Spiegel-Experiment machen und zunächst \textit{u}, \textit{o}, \textit{ü} und \textit{ö} sprechen und dann \textit{a}, \textit{i}, \textit{e} und \textit{ä}, dann werden Sie beobachten, dass bei der Artikulation der ersten Gruppe die Lippen gerundet sind, bei der zweiten Gruppe aber nicht.}
...
Man kann die Vokale in einem Vokalviereck (manchmal auch Vokaltrapez genannt) zusammenfassen.
...
%Die Pfeile markieren die Diphthonge, die in Abschnitt~\ref{sec:diphthonge} erklärt werden.
%Es ist bei der physiologischen Interpretation des Vokalvierecks allerdings Vertiefung \ref{vert:mri} zu beachten.
...
\begin{table}[!h]
  ...
  \begin{tabular}{cccccc}
    ...
    \lspbottomrule
  \end{tabular}
%  \ncline[nodesep=3pt]{->}{a1}{a2}
%  \ncline[nodesep=3pt]{->}{a1}{a4}
%  \ncline[nodesep=3pt]{->}{a4}{a10}
  \caption{IPA:\ Vokalviereck für das Deutsche}
  \label{tab:vokaltrap}
\end{table}

%\BVertiefung{Vokale und Zungenposition}{
%\label{vert:mri}
%Dass das Vokalviereck eine vergleichsweise starke Abstraktion ist, kann man sehen, wenn man Aufnahmen der Zungenposition bei den entsprechenden Artikulationen betrachtet.
%Solche Aufnahmen sind in Abbildung~\ref{fig:mrie} für \textipa{[E]} und \textit{e:} und in Abbildung~\ref{fig:mrio} für \textipa{[O]} und \textipa{[o:]} zu sehen.
%Die Aufnahmen stammen aus Echtzeit-MRI-Filmen, die von den Autoren von \citet{NiebergallEa2013} freundlicherweise zur Verfügung gestellt wurden.
%Zum Vergleich zeigt Abbildung~\ref{fig:mrit} die Position in der Verschlussphase von \textipa{[t]} und die Ruheposition mit geschlossenem Mund.
%
%Man erkennt deutlich, dass die Zunge nicht eine plane Fläche bildet, die im Mundraum auf zwei Achsen verschoben wird.
%Die gesamte Form der Zunge (nicht nur der Zungenspitze und des Zungenrückens) verändert sich teilweise drastisch.
%Sehr gut erkennt man auch die unterschiedlichen Grade der Lippenöffnung und das gehobene Velum auf allen Aufnahmen außer in der Ruheposition.
%Die Vorgänge im Kehlkopf lassen sich leider auf MRI-Aufnahmen nicht differenziert erkennen.
%}
%
%\begin{figure}[h!]
%  \centering
%  \includegraphics[width=0.4\textwidth]{figures/kurz-e-bett}
%  \includegraphics[width=0.4\textwidth]{figures/lang-e-beet}
%  \caption{MRI-Aufnahmen der Vokale in \textit{Bett} und \textit{Beet}}
%  \label{fig:mrie}
%\end{figure}
%
%\begin{figure}[h!]
%  \centering
%  \includegraphics[width=0.4\textwidth]{figures/kurz-o-post}
%  \includegraphics[width=0.4\textwidth]{figures/lang-o-boot}
%  \caption{MRI-Aufnahmen der Vokale in \textit{Post} und \textit{Boot}}
%  \label{fig:mrio}
%\end{figure}
%
%\begin{figure}[h!]
%  \centering
%  \includegraphics[width=0.4\textwidth]{figures/t-hat}
%  \includegraphics[width=0.4\textwidth]{figures/ruheposition}
%  \caption{MRI-Aufnahmen von [t] in \textit{hat} und der Ruheposition}
%  \label{fig:mrit}
%\end{figure}
...
Phonetisch ist ein mit dem Zeichen \textit{n} geschriebener Laut nicht immer ein \textipa{[n]}.

\Tu{Sprechen Sie die Wörter in (\ref{ex:phot7303}) langsam aus und achten Sie auf den Artikulationsort des mit \textit{n} geschriebenen Sprachlauts.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{\textipa{[PUmpas@nt]} oder \textipa{[PUnpas@nt]}, \textipa{[PUmbIl]} oder \textipa{[PUnbIl]}}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Mündel \textipa{[mYnd\s{l}]} \slash \textipa{[mYnd@l]}}
  \end{xlist}
\end{exe}
...
\Tu{Lesen Sie die Wörter in (\ref{ex:phot1112}) laut vor und achten Sie auf die Stimmhaftigkeit der \textit{s}-Laute.
