...
In erster Näherung entsprechen Buchstaben wie \textit{a} oder \textit{t} in einer Buchstabenschrift wie der deutschen durchaus prototypischerweise einem Laut. 
...
Als Nebeneffekt wird in \textit{hasse} der \textit{ss}-Laut stimmlos (s.\ Abschnitt~\ref{sec:stimmhaftigkeit}) ausgesprochen, der \textit{s}-Laut in \textit{Hase} aber stimmhaft.
Impressionistisch gesagt klingt das \textit{ss} in \textit{hasse} \textit{härter} als das \textit{s} in \textit{Hase}.

Im Gegensatz dazu sind die Diphthongschreibungen \textit{ei} (\textit{frei}) und \textit{eu} (\textit{neu}) ein gutes Beispiel für eine synchron unmotivierte Korrespondenz von Laut und Buchstabe.
Diphthonge sind Kombinationen aus zwei Vokalen, um die es in Abschnitt~\ref{sec:diphthonge} genauer geht.
Wollten wir diese Diphthonge jedenfalls so schreiben, dass die Buchstaben in ihnen so gelesen werden, wie sie sonst auch gelesen werden, müssten wir \textit{ai} (\textit{*frai}) und \textit{oi} (\textit{*noi}) schreiben.
...
Hinzu kommt die Schreibung \textit{äu} (\textit{Mäuse}), die wie \textit{eu} gelesen wird, und die im Gegensatz zu \textit{eu} tief im grammatisch-graphematischen System verankert ist (Abschnitt~\ref{sec:konstanz}).
...
Der \textit{t}-Laut besteht (wie unten genau gezeigt wird) aus einer kurzen Phase der Stille, gefolgt von einem kurzen Knallgeräusch und ggf.\ einem Entweichen von Luft durch den Mundraum.
...
Es werden Merkmale wie \textsc{(Artikulations)art} (Abschnitt~\ref{sec:artikulationsart}) und \textsc{(Artikulations)ort} (Abschnitt~\ref{sec:artort}) beschrieben, ohne dass die Merkmalsschreibweise benutzt wird.
In Abschnitt~\ref{sec:photpholmerkmale} werden die Merkmale abschließend zusammengefasst werden.
...
\Tu{Wenn Sie sich an den Kehlkopf/die Kehlkopfgegend fassen und verschiedene Wörter langsam sprechen (\zB \textit{Achat}, \textit{Verwaltungsangestellter}), werden Sie merken, dass der Kehlkopf bei einigen Segmenten (\textit{a}, \textit{w}, \textit{ng} usw.) eine Vibration produziert, bei anderen (\textit{ch}, \textit{t} usw.) aber nicht.}
...
\Tu{Sprechen Sie Wörter mit vielen \textit{h}-Segmenten am Silbenanfang aus, \zB \textit{Haha}, \textit{Hundehalter} usw.
...
In Zusammenspiel mit der hinteren Zunge ist der Rachen in vielen Sprachen (\zB im Arabischen) an der Produktion von Segmenten beteiligt, im Standarddeutschen allerdings nicht.
...
Der bewegliche Teil der Zunge ist als aktiver Artikulator (s.\ Abschnitt~\ref{sec:artikulator}) essentiell für die Bildung vieler Segmente.
...
Bei der zuerst zu besprechenden Gruppe von Segmenten handelt es sich um die sogenannten Obstruenten.
...
Das Geräusch, das bei diesen Segmenten entsteht, kann als Rauschen (oder Reibegeräusch) beschrieben werden.
Diese Laute nennt man daher \textit{Frikative} oder \textit{Reibelaute}.
...
Die Segmente sind also kontinuierlicher als Plosive.
...
Affrikaten sind gewissermaßen komplexe Segmente, nämlich eine direkte Abfolge von einem Plosiv und einem Frikativ.
...
Die deutschen \textit{pf}-Segmente sind \zB streng genommen nicht homorgan, wie dort erläutert wird.
...
Im Deutschen ist das \textit{l}-Segment der einzige laterale Approximant.

\Tu{Beobachten Sie, wie im Wort \textit{Ball} das letzte Segment gebildet wird.}
...
Bei diesen beiden Segmenten handelt es sich um Nasale.
...
Somit wird klar, warum diese Segmente nicht mit geschlossener Nase auszusprechen sind:
...
Da wir einen intuitiven Begriff von Vokalen haben und die orthographisch als \textit{a}, \textit{e}, \textit{i}, \textit{o}, \textit{u} sowie \textit{ä}, \textit{ö}, \textit{ü} und ggf.\ \textit{y} wiedergegebenen Segmente als Vokale bereits kennen, können wir überlegen, was das Besondere an ihnen ist.

\Tu{Sprechen Sie sich die Vokalsegmente vor und beobachten Sie dabei (einschließlich Beobachtung im Spiegel), wie sich die Zunge, die Lippen und die sonstigen Organe im Mundraum dabei verhalten.}

Die Zunge bewegt sich bei der Artikulation verschiedener Vokale im Mund\-raum zu verschiedenen Positionen, aber es findet bei keinem der Segmente eine deutliche Verengung an irgendeinem Artikulator statt, und der Luftstrom kann daher weitgehend ungehindert abfließen.
...
Im Bereich des Kehlkopfs (Larynx) bilden Sprecher des Standarddeutschen zwei verschiedene Segmente.
...
Vor dem Spiegel sieht man gleich, dass Wörter wie \textit{Pass} oder \textit{Ball} mit einem an der gleichen Stelle artikulierten Segment beginnen.
...
Während bei den zuletzt genannten Segmenten beide Lippen beteiligt sind (daher der Terminus \textit{bilabial}), erkennt man bei den Anlauten von \textit{Fuß} und \textit{Wade} sofort, dass die Zähne des Oberkiefers beteiligt sind, die sich an die Unterlippe legen.
...
Man schreibt im IPA-Alphabet daher diese Segmente mit den Grundzeichen für den Plosiv und den Frikativ mit einem verbindenden Bogen (Ligatur).
...
\footnote{Beispielwörter werden ab jetzt vollständig transkribiert, auch wenn die genaue Notation der Vokale erst im Laufe dieses Abschnitts geklärt wird.}
...
Dies liegt daran, dass es für Vokale keinen gut lokalisierbaren einzelnen Artikulationsort gibt und die Orientierung im Mundraum dadurch erschwert wird.
...
Die Stufen dazwischen nennt man dann \textit{halbvorne}, \textit{halbhinten} und \textit{halbhoch}, \textit{halbtief}.
...
In der Tabelle findet sich ein besonders charakteristisches Segment, nämlich das sogenannte \textit{Schwa} \textipa{[@]}.
Das Schwa ist ein \textit{Zentralvokal} (er steht in jeder Hinsicht in der Mitte der Vokalvierecks.
...
Alle anderen Vokale sind immer kurz, auch wenn sie betont werden (\textipa{[I]} wie in \textit{Rinder} \textipa{[KInd@]}).
...
In der Phonologie (Kapitel~\ref{sec:phonologie}) werden in diesem Buch diese phonetischen Merkmale benutzt.
...
Vielmehr war das weitaus bescheidenere Ziel, Lesern und Leserinnen einen Überblick über die Segmente zu geben, die im in Deutschland gesprochenen Standarddeutschen vorkommen.
...
In diesem Abschnitt werden daher einige Besonderheiten besprochen, die gerne zu Problemen bei der Pseudo-Transkription der Deutschen führen.
...
Diese ist ein typischer, aber nicht exklusiver phonologischer Prozess im Deutschen
...
\Tu{Sprechen Sie die Wörter in (\ref{ex:phot7303}) langsam aus und achten Sie auf den Artikulationsort des mit \textit{n} geschriebenen Segments.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{\textipa{[PUmpas@nt]} oder (empfohlen) \textipa{[PUnpas@nt]}, \textipa{[PUmbIl]} oder (empfohlen) \textipa{[PUnbIl]}}
...
\Tu{Lesen Sie die Wörter in (\ref{ex:phot1112}) laut vor und achten Sie auf die Stimmhaftigkeit der \textit{s}-Segmente.
