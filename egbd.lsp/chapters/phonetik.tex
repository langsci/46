...
\Zusammenfassung{
Wir berücksichtigen hier nur die \textit{artikulatorische Phonetik}, die die Produktion von Sprachlauten mittels der \textit{Sprechorgane} beschreibt.
Die Betrachtungseinheit ist dabei das \textit{Segment}, ein Laut, dessen weitere Unterteilung keinen Beschreibungsvorteil mit sich brächte.
Die Beziehung zwischen der Aussprache (\textit{Phonetik}) und der Schreibung ist kompliziert und muss im Rahmen der \textit{Graphematik} gesondert diskutiert werden.\index{Graphematik}
Wir sprechen auf jeden Fall nicht so, wie wir schreiben.
}
...
\Zusammenfassung{
  Neben der \textit{Lunge}, die den nötigen \textit{Luftstrom} erzeugt, ist der \textit{Kehlkopf}, der den \textit{Stimmton} produziert, für die grundlegenden Mechanismen bei der Sprachproduktion (\textit{Obstruktion} des Luftstroms) verantwortlich.
Außerdem sind Organe zwischen \textit{Rachen} und den \textit{Lippen} beteiligt.
}
...
Ein Artikulator ist ein Organ, das an einer Artikulation beteiligt ist.
Der \textit{aktive} Artikulator führt dabei eine Bewegung zu dem unbeweglichen \textit{passiven} Artikulator aus.
\index{Artikulator}
}
...
\textit{Was} die Artikulatoren bei welchen Segmenten genau machen, wird \textit{Artikulationsart} (Definition~\ref{def:artart}) genannt und in den folgenden Abschnitten klassifiziert und illustriert.
...
\Zusammenfassung{
Die \textit{Artikulationsart} beschreibt die Art, in der die Sprechorgane die Sprachlaute produzieren und verändern.
Neben der \textit{Stimmhaftigkeit} ist es relevant, wie stark sich die \textit{Artikulatoren} einander annähern bzw.\ einen Verschluss herstellen.
}
...
Man unterscheidet gemeinhin eine \textit{enge Transkription} von einer \textit{weiten oder lockeren Transkription}.\index{Transkription!eng und weit}
...
ist jeweils der zwischen ungerundeten und gerundeten Vokalen.
...
Alle (halb-)hinteren Vokale (\textipa{[U]}, \textipa{[u]}, \textipa{[o]}, \textipa{[O]}) sind prinzipiell gerundet.
Alle zentralen Vokale sind prinzipiell ungerundet (\textipa{[@]}, \textipa{[5]}, \textipa{[a]}).
}
...
Die längere Variante kommt in betonten Silben vor (\textipa{[i:]} in \textit{Liebe} \textipa{[li:b@]}, \textipa{[e:]} wie in \textit{Weg} \textipa{[ve:k]}), die kürzere in unbetonten (\textipa{[i]} und \textipa{[o]} in \textit{Lithographie} \textipa{[litogKafi:]}, \textipa{[e]} wie in \textit{Methyl} \textipa{[mety:l]}).
Alle anderen Vokale sind immer kurz, auch wenn sie betont werden (\textipa{[I]} wie in \textit{Rinde} \textipa{[KInd@]}).
In Abschnitt~\ref{sec:gespanntheitbetonunglaenge} wird eine Analyse dieser komplizierteren Längenverhältnisse vorgeschlagen.

\index{Schwa}
In Abbildung~\ref{fig:vokaltrap} findet sich schließlich noch ein besonderes Segment, nämlich das sogenannte \textit{Schwa} \textipa{[@]}.
...
\Zusammenfassung{
Für Konsonanten ist der \textit{Artikulationsort} als die Stelle definiert, an der sich die \textit{Artikulatoren} einander annähern oder einen Verschluss herstellen, \zB am Zäpfchen, Zahndamm, an den Lippen.
...
Auch Oberklassen wie die der \textit{Obstruenten} kann man als Merkmale kodieren.
}
...
\Zusammenfassung{
  Im Deutschen sind Obstruenten im Silbenendenauslaut immer stimmlos (\textit{Auslautverhärtung}).
  Außerdem gibt es keine vokalisch anlautenden Wörter (Einfügung des \textit{Glottalplosivs}).
  Der \textit{r}-Laut wird nur am Silbenanfang \textipa{[K]} ausgesprochen, am Silbenende bildet er mit dem vorangehenden Vokal einen sekundären Diphthong.
}
