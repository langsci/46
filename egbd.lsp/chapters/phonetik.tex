...
Vor allem um die Regularitäten der Symbolkodierung in der deutschen Schreibung geht es in Teil~\ref{part:schrift} dieses Buchs.
...
Sie sind durch Muskelkraft kontrolliert bewegbar, und an ihnen sind die \textit{Stimmbänder} aufgehängt, die die \textit{Simmlippen bilden}, s.\ Abbildung~(\ref{fig:glottis}).
...
\begin{figure}
  \includegraphics[width=0.5\textwidth]{figures/larynx}
  \caption[Kehlkopf von oben]{Kehlkopf von oben (Quelle: Alan Hoofring\slash National Cancer Institute, Public Domain)}
  \label{fig:glottis}
\end{figure}
