...
Teil~\ref{part:schrift} des Buchs geht dann detaillierter auf die Schreibprinzipien des Deutschen ein.
...
Zur weiteren Differenzierung des Gaumenbereichs spricht man bei der Stufe direkt hinter den Schneidezähnen vom \textit{Zahndamm} (die \textit{Alveolen}).\index{Zahndamm}
...
Was die Artikulatoren bei welchen Segmenten genau machen, wird \textit{Artikulationsart} genannt und in den folgenden Abschnitten klassifiziert und illustriert.
...
Man unterscheidet gemeinhin eine enge Transkription von einer weiten oder lockeren Transkription.\index{Transkription!eng und weit}
...
\Zusammenfassung{
Man kann die wichtigen Artikulationen im standardnahen Deutsch mit sechs Merkmalen abbilden.
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex{Bündel \textipa{[bYnd\s{l}]} oder \textipa{[bYnd@l]}}
  \end{xlist}
\end{exe}
...
Der Nasal \textipa{[n]} passt sich in seinem Artikulationsort innerhalb eines Wortes immer an die nachfolgenden Plosive \textipa{[k]} und \textipa{[g]} an.
Im Fall von \textit{ungenau} ergeben sich Schwankungen zwischen einer angepassten Variante \textipa{[PUNgen\t{aO}]} und einer nicht angepassten Variante \textipa{[PUngen\t{aO}]}, weil sich nach der Vorsilbe \textit{un-} ggf.\ eine besondere Grenze innerhalb des Wortes befindet.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex{\textipa{[klINk@]}, \textipa{[baNk]}, \textipa{[PUng@n\t{aO}]} bzw.\ \textipa{[PUNg@n\t{aO}]}}
