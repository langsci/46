...
Man kann Verben so versteht, dass sie ein Ereignis (\zB \textit{kaufen}) oder einen Zustand (\zB \textit{liegen}) bezeichnen, wobei jetzt für Ereignisse und Zustände mit einem Sammelbegriff von \textit{Situationen} gesprochen wird.
...
\Zusammenfassung{
\textit{Semantische Rollen} klassifizieren die Mitspieler an einer durch ein Verb beschriebenen Situation, \zB nach \textit{Agentivität}, aber es gibt keine vom Verbtyp unabhängige Beziehung zwischen Rollen und Kasus.
}
...
Eine ausführliche Betrachtung ist angesichts der Anlage dieses Buchs nicht möglich.
...
Das Prädikat bestünde dann aus dem finiten Verb und allen von ihm abhängigen Konstituenten außer den Satzgliedern (vgl.\ Abschnitt~\ref{sec:satzglied}).
...
\Zusammenfassung{
Das \textit{Satzprädikat} ist schwer zu definieren (am ehesten noch als die direkt voneinander abhängigen finiten und infiniten Verbformen eines Satzes).
\textit{Prädikative Konstituenten} sind eine heterogene Klasse, lassen sich aber auf den Prototyp der zweiten \textit{Kopula-Ergänzung} zurückführen.
}
...
Die beiden Sätze enthalten aber keine Verben bzw.\ Konstruktionen, in denen sogenannte Prädikatsnominative (s.\ Vertiefung~\ref{vert:praednom} auf S.~\pageref{vert:praednom}) vorkommen, so dass eine andere Erklärung für den doppelten Nominativ gefunden werden muss.
...
\Zusammenfassung{
\textit{Subjekte} sind die \textit{Nominativ-Ergänzungen} von Verben oder Nebensätze (und Ähnliches), die deren Stelle einnehmen.
Es gibt mindestens vier verschiedene \textit{es}: normales \textit{Pronomen}, \textit{Korrelat}, \textit{fakultative} bzw.\ \textit{obligatorische spezifische Ergänzung} und  \textit{positionales} \textit{es}.
}
...
\Zusammenfassung{
\textit{Passive} mit \textit{werden} und \textit{bekommen} können prototypisch von Verben mit agentivem Nominativ gebildet werden, wobei dieser Nominativ zur optionalen \textit{von}-PP wird und entweder der Akkusativ (bei \textit{werden}) oder der Dativ (bei \textit{bekommen}) zum Nominativ des Passivs wird.
Es gibt zwei Arten von \textit{intransitiven Verben} (einschließlich Nominativ-Dativ-Verben), nämlich \textit{Unakkusative} mit nicht-agentivem Nominativ wie \textit{platzen} und \textit{Unergative} mit agentivem Nominativ wie \textit{arbeiten}.
}
...
In diesem Abschnitt wird abschließend der Ergänzungsstatus verschiedener Dative diskutiert und dabei der Begriff des \textit{indirekten Objekts} definiert.
...
\Satz{Ergänzungen und Angaben im Dativ}{
\label{def:freidat}
...
\Zusammenfassung{
Dative, mit denen ein \textit{bekommen}-Passiv gebildet werden können (einschließlich \textit{Pertinenzdativ} und \textit{Nutznießerdativ}), sind keine Angaben.
PPs mit einer nicht vom Verb zugewiesenen Rolle sind Angaben und können mit dem \textit{Paraphrasentest} aus dem Satz extrahiert werden.
}
...
Das Plusquamperfekt ist ein Perfekt im Präteritum.
Das traditionelle Perfekt wie in (\ref{ex:prd3291a}) heißt korrekt \textit{Präsensperfekt} und steht neben dem \textit{Präteritumsperfekt} wie in (\ref{ex:prd3292b}).
...
Die Konstituentenklammern in (\ref{ex:prd3291}) und (\ref{ex:prd3292}) deuten an, dass es sich hier um Verbkomplexbildung handelt, und dass innerhalb des Verbkomplexes die Verben paarweise kombiniert werden, so dass jedes Hilfsverb mit dem Verb kombiniert wird, dessen Status es regiert.
...
Während das Präteritum in (\ref{ex:prd7452b}) nur so verstanden werden kann, dass es 1993 ein punktuelles Ereignis gab, lässt das Präsensperfekt in (\ref{ex:prd7452a}) neben dieser Lesart eine zweite zu.
...
Nur sehr kurz kann hier angemerkt werden, dass auch das Passiv infinite Formen hat, wenn das Passiv-Hilfsverb infinit ist.
...
\Zusammenfassung{
\textit{Futurperfekt} und \textit{Präteritumsperfekt} sind keine eigenständigen Tempora, sondern analytisch auf Basis des Perfekts und des Präteritums bzw.\ des Futurs gebildet.
}
...
Es folgt die relevante Definition~\ref{def:kohaerenz}.

\Definition{Kohärenz}{
\label{def:kohaerenz}
Ein Verb A, das ein infinites Verb B regiert, konstruiert kohärent, wenn A und B einen Verbkomplex bilden.
...
Die Identität zwischen dem Subjekt des Kontrollverbs und dem nicht ausgedrückten des regierten Verbs (\textit{zu}-Infinitiv) kommt nicht über eine Vereinigung der Valenzlisten zustande, sondern über eine besondere Relation (die \textit{Kontrollrelation}), die im folgenden Abschnitt genauer besprochen wird.


\Zusammenfassung{
In der \textit{kohärenten Konstruktion} bildet ein regierendes Verb mit seinem regierten infiniten Verb einen Verbalkomplex.
Bei der \textit{inkohärenten Konstruktion} bildet das regierte Verb eine eigene VP.
\textit{Modalverben} (\textit{wollen}) konstruieren \textit{obligatorisch kohärent} und verschmelzen typischerweise ihre Subjektanforderung mit der des regierten Verbs.
\textit{Halbmodalverben} (\textit{scheinen}) konstruieren \textit{obligatorisch kohärent} und übernehmen die Kasus- und Rollenanforderung des regierten Verbs vollständig.
}
...
Die Definition von \textit{Infinitivkontrolle} erfasst jetzt bereits alle wesentlichen Fälle von Kontrolle, von denen einige dann im Folgenden erst bebeispielt werden.
...
\Zusammenfassung{
Bei \textit{Kontrollverben} wird die Bedeutung des nicht ausgedrückten Subjekts der eingebetteten VP von anderen nominalen Valenzstellen des regierenden Verbs beigesteuert.
...
\Zusammenfassung{
Die \textit{Bindungstheorie} beschreibt die Beschränkungen der Interpretationsmöglichkeiten von normalen Pronomina und Reflexivpronomina in bestimmten syntaktischen Strukturen.
}
