...
Mitspieler können konkrete oder abstrakte Gegenstände (einschließlich Menschen), andere Situationen usw.\ sein.
\index{Rolle}
\index{Mitspieler}
}
...
Unabhängig davon muss man annehmen, dass die Präpositionen in PP-Angaben den regierten PPs eine Rolle zuweisen, und dass der Kasus freier NP-Angaben direkter Ausdruck einer freien Rolle ist.\index{Angabe!präpositional}
...
In diesem Kapitel werden einige besondere Formen von sog.\ Prädikaten bzw.\ deren Bildung sehr kurz angesprochen.
...
Ein häufig anzutreffender Begriff, der vom Prädikatsbegriff abgeleitet ist, ist der des Prädikativums, der Prädikativergänzung, Prädikativangabe usw.
...
Die vorkommenden NP, PP usw.\ werden als prädikative NP, prädikative PP usw.\ bezeichnet.
\index{Prädikativ}
}
...
\begin{exe}
  ...
  \begin{xlist}
    ...
    \ex[*]{\label{ex:rel23432a} Backt einen Kuchen.}
    \ex[*]{\label{ex:rel23432a} Einen Kuchen backt.}
    \ex[ ]{\label{ex:rel23432c} [Herr Uhl] raucht.}
    \ex[*]{\label{ex:rel23432c} Raucht.}
    \ex[ ]{\label{ex:rel23432e} [Es] regnet.}
    \ex[*]{\label{ex:rel23432e} Regnet.}
  \end{xlist}
\end{exe}
...
Das Subjekt ist also im Kern mit der Nominativergänzung des Verbs identisch.

\index{Komplementsatz}

\Definition{Subjekt}{
\label{def:subjekt}
Das Subjekt ist die Nominativergänzung oder eine satzförmige Konstituente, die anstelle einer Nominativergänzung steht.
Die sogenannte Subjekt-Verb-Kongruenz besteht zwischen dem regierten Nominativ und dem regierenden finiten Verb.
Komplementsätze und Infinitivkonstruktionen als Subjekte (\zB sog.\ Subjektsätze) haben keine Merkmale, mit denen das finite Verb kongruieren könnte.
...
\begin{exe}
  ...
  \begin{xlist}
    \ex[]{\label{ex:prde0001} Mir geht es gut.}
    \ex[*]{\label{ex:prde0001} Mir geht gut.}
  \end{xlist}
\end{exe}
...
\begin{figure}[h!]
    \Treek[3]{4}{
      & \K{Ersetzbar durch \textit{dieses}?}\QQwwww\DLqQ[dl]_{\mathrm{Ja}}\DLqQ[dr]^{\mathrm{Nein}} \\
      \K{normales}\Below{Pronomen} && \K{Mit Komplementsatz?}\QQwwww\DLqQ_{\mathrm{Ja}}\DRqQ^{\mathrm{Nein}} \\
      & \K{Korrelat} && \K{Obligatorisch im Vf?}\QQwwww\DLqQ_{\mathrm{Ja}}\DRqQ^{\mathrm{Nein}} \\
      && \K{fak.\ positionales \textit{es}}\Below{(Vorfeld-\textit{es})} && \K{obl.\ oder fak.\ Ergänzung}\Below{(Wetter\slash Experiencer"=Verben)} \\
    }
...
Außer dass das Vollverb im Partizip auftritt (\textit{schenkt}--\textit{geschenkt} usw.) und das Hilfsverb \textit{werden} als finites Verb hinzukommt, ergibt sich ein relativ klares Muster bezüglich der Valenzänderungen vom Aktivverb zum zugehörigen Passivverb.
...
\index{Agens}
Die hier verwendeten Definitionen für Agens und für das \textit{werden}-Passiv erlauben es, die Passivierbarkeit eines Verbs als Zeichen dafür zu verwenden, dass das Verb ein agentives Subjekt hat.
...
Der Bewertungsdativ ist im V2-Satz auf der Position nach dem finiten Verb (also ganz am Anfang des Mittelfelds) festgelegt (die sog.\ \textit{Wackernagel-Position}).
